#!/usr/bin/env node
/* =====================================================================
   tools/verify/shots-ics14d.js — ICS-14d UNIFORM ROBOTO STREET-TREATMENT
   shots (operator directive 2026-07-16: every label voice = Roboto, white
   ink + dark-grey casing, no glow; legibility at a glance is the only
   goal). The operator's three framings, on the RELEASE target:
     closeup-150m    @ 1849-12-20 — building closeup, owner names + lot
                       numerals on the ground, business names over roofs
     waterfront-450m @ 1849-12-20 — mid-zoom Montgomery waterfront,
                       business/civic names over sand + roofs
     overview-1500m  @ 1849-09-15 — high overview, hill/district caps
   Also prints the LIVE labels.contrast per-background decision table for
   EVERY voice (the ics14d measurement record).
   Usage: node tools/verify/shots-ics14d.js [prefix]   (default "after")
   Saves under app/screenings/2026-07-16-ics14d/<prefix>-<id>.png.
...
const OUT_DIR = path.join(APP_ROOT, "screenings", "2026-07-16-ics14d");
const PREFIX = process.argv[2] || "after";

const SHOTS = [
  { id: "closeup-150m-1849-12-20",    alt: 150,                 date: "1849-12-20", note: "building closeup — owner names + lot numerals + business names on roofs" },
  { id: "waterfront-450m-1849-12-20", alt: 450, x: 180, z: -70, date: "1849-12-20", note: "mid-zoom Montgomery waterfront — business names over sand + roofs" },
  { id: "overview-1500m-1849-09-15",  alt: 1500,                date: "1849-09-15", note: "high overview — hill/district caps" }
];
...
(async () => {
  ...
  // give the embedded FontFace load + full-voice re-raster a beat to complete
  ...
  // ---- ICS-14d measurement record: every voice's per-background table ----
  ...
  if (contrast) {
    ...
    (d.styles || []).forEach(s => {
      console.log(`  ${String(s.style).padEnd(7)} ALLmin ${s.minRatio} @${s.worstBg}   HOSTmin ${s.hostMin} @${s.hostWorstBg}   host=${JSON.stringify(s.hostFam)}`);
      (s.perBg || []).forEach(p => console.log(`    ${p.bg.padEnd(10)} ${p.host ? "HOST" : "    "} inkVsCasing=${p.inkVsCasing} casingVsBg=${p.casingVsBg} inkVsBg=${p.inkVsBg} -> ${p.score}`));
    });
  }
