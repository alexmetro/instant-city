...
(function registerEdgeGroundingAudits(){
  ...
  /* spine.wharfFootDryInland — GATING (s110d Phase 2c). Each pier/wharf foot
     anchors on DRY LAND (isDryLand — above every tide plus the freeboard) and
     is set back >= ~8 ft (2.4 m) INLAND of the dry-land edge, NEVER at the
     coastline/waterline; the deck then runs seaward over water. A pier with
     no active checkpoint at this date is skipped lawfully (date-gated, like
     the rest of the pier family). Broadway's re-anchored foot (s110d, grid
     u=300.2 — measured 2.45 m inland of the isDryLand crossing at u=302.65)
     passes by construction; Central's foot sits ~82 m inland. The pre-s110d
     Broadway foot (u=320, h~+0.05 m — dry at mean tide, submerged at high
     tide) FAILS this rule: the proof it works. */
  registerAudit("spine", "wharfFootDryInland", function(){
    var out = { pass:true, gating:true, wharvesChecked:0, misAnchored:0, misAnchoredIds:[], detail:[] };
    var edge = dryLandEdgeAt(simDay);
    livePiers().forEach(function(p){
      var active = pierActiveCheckpoint(p, simDay); if(!active) return;   // absent/reaching at this date — lawful date-gated skip
      out.wharvesChecked++;
    ...
    out.pass = out.misAnchored === 0;
    out.note = "GATING (Phase 2c): "+out.misAnchored+" of "+out.wharvesChecked+" active wharf feet fail the dry-inland anchor rule.";
    return out;
  });
})();
