...
var FILL_QA = { ignoreInTownGates: false, uncapCoreCanvas: false, version: 0 };
...
function buildFillMaster(){
  ...
  while(waveN < need2){
    ...
    for(; idx2 < slots2.length; idx2++){
      ...
      counts2[cat2]++; waveN++;
      if(fillCoreBlockAt(cx2, cz2, waveCap)) coreWaveN++;   // s106d: core-block placements feed the streaming canvas cap (same ground measure as the audit)
...
function fillLifecycleOf(e){
  ...
  } else if(!fire && e.cat === "shanty" && lcHash(id, "upg") < 0.30){
...
function rebuildFill(){
  ...
  for(var i = 0; i < set.length; i++){
    ...
    if(set[i].subtype === "tent" && set[i].cls !== "frame" && typeof TENT_VARIANTS !== "undefined"){ tentPartsInto(tentParts, set[i]); continue; }
...
if(typeof window !== "undefined"){
  ...
  window.__P1850_FILL_QA = function(opts){
    ...
    return { ignoreInTownGates: FILL_QA.ignoreInTownGates, uncapCoreCanvas: FILL_QA.uncapCoreCanvas, version: FILL_QA.version };
  };
}
setTimeout(function(){
  if(typeof window !== "undefined" && window.__P1850){
