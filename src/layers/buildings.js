...
function fillCensusCurve(){
  ...
  _fillCensusCurve = [
    ...
    { day: eventDateToSimDay("1849-12-20"), n: 765 }   // ics26 count flex INSIDE the s106c band [630,780] (was the 720 mid-point): the RES-4 measured coverage curve needs the extra ~45 units of footprint to reach the per-band aggregates; the interpolation itself allowed ~700-750+ and the gating plausibility band is unchanged
...
/* seeded weighted draw of a concrete code within a census category, era-valid at
   ...
   s106d passes tentOnly for the core-canvas sub-walk — the boom-window core
   exception admits CANVAS, never the scrap-shanty tier; ics26 passes areaBias
   — the size-grading exponent (fill-density-model §7: dense core ground
   weights medium/large codes; sparse fringe weights small): pick weight is
   multiplied by (footprint_ft2/300)^areaBias, 0 = the historic behaviour). */
function fillPickCode(cat, day, rng, snapKey, tentOnly, areaBias){
  var pool = [], tot = 0;
  for(var i = 0; i < FILL_TYPES.length; i++){
    var t = FILL_TYPES[i];
    if(t.isGapMarker) continue;
    if(!FILL_COUNTED_SUBTYPES[t.subtype]) continue;
    if(tentOnly && t.subtype !== "tent") continue;
    ...
    if(areaBias) w *= Math.pow((t.footprint.w * t.footprint.d) / 300, areaBias);   // ics26 size grading
...
/* ics26 — THE YARD-FENCE REGISTRY (activates the placement.fences P6 stub):
   FENCE_SEGS/FENCE_POSTS are the audit's data surface (06-debug reads these
   module globals); FILL_FENCE_RENDER carries the per-lot render bundles
   (appearDay-gated placeholder rails). Rebuilt with the fill master. */
var FENCE_SEGS = [], FENCE_POSTS = [], FILL_FENCE_RENDER = [];
...
/* s106e — OBB (quad) SAT penetration in metres, 0 if separated (0.05 slack —
   the same measure the fill audits use). Module-level because the expansion
   post-pass in buildFillMaster must prove an annex claim legal against every
   other footprint at master-build time (packing law). */
function fillQuadPen(A, B){
  var polys = [A, B], best = 1e9, p, i, k;
  for(p = 0; p < 2; p++){ var poly = polys[p];
    for(i = 0; i < 4; i++){ var a = poly[i], b = poly[(i+1)%4];
      var ax = -(b.z - a.z), az = (b.x - a.x), l = Math.hypot(ax, az) || 1; ax /= l; az /= l;
...
var FILL_QA = { ignoreInTownGates: false, uncapCoreCanvas: false, ignoreSlotGates: false,
                ignoreCoverageTargets: false,   // ics26: revert the wave to the flat plaza-radial fill (no hotspot fields / block budgets / size grading) — buildings.blockCoverage fail-before
                version: 0 };
...
/* s106d (ICS-24) tunables — THE BOOM-WINDOW CORE-CANVAS EXCEPTION. The zone
   law's window + fraction live in core/08-cadastre (CAD_CORE_CANVAS_*, ONE
   set of constants the gate, this spawner and the zoneCoreCanvas audit all
   read — measurement law). Here: the SHORT lifecycle arc — a core tent is the
   FIRST torn down and replaced by frame (the s108 teardown-replacement
   machinery), seeded 2-5 months after its own spawn so the conversions spread
   across late-1849 -> mid-1850 and scrubbing shows the core visibly shedding
   canvas (fill:true reasoned; the record's precedent is the El Dorado's
   authored canvas phase on the plaza, torn down for its frame expansion). */
var FILL_CORE_CONV_MIN_D = 60;            // earliest seeded canvas->frame conversion after spawn (~2 months)
var FILL_CORE_CONV_MAX_D = 150;           // latest (~5 months): a Dec-20 spawn is torn down by ~mid-May 1850 — zero core canvas stands by mid-1850
/* s106e (ICS-25) tunables — CANVAS-FIRST LOT ARCS + SLOTS (fill:true, operator
   design). TWO mechanics, ONE system:
   (1) CANVAS-FIRST ARCS — a boom-era fill lot's development is an ARC:
       empty -> canvas (short) -> its final form. A seeded fraction of the
       1849 wave's RESIDENTIAL-band placements (never the core — the core
       keeps the s106d 60-150d law; never the encampment zones — their own
       record-wins law is untouched) BEGIN as canvas and HARDEN into their
       final catalog form via the existing s108 teardown-for-replacement
       machinery, a zone-modulated interval after THEIR OWN spawn. This
       changes the PATH a structure takes, never its destination: the
       master's counts/mix at the noon dates are untouched by construction
       (the entry's rank/cat/footprint are placement-time facts; the canvas
       phase is a lifecycle read).
   (2) SLOTS — a wave fill lot is a small set of POSITIONS by size class:
       the street-front slot (the wave placement itself) + 1-2 rear tandem
       slots reaching into the block (rear-court texture, fill-density-model
       §3: "one street building + several smaller buildings on an internal
       court"). Each slot runs its OWN seeded lifecycle arc; rear occupants
       are TENTS (canvas) drawn from the wave's shanty bucket — a rear
       placement CONSUMES a shanty turn, so the wave's total and its 55/45
       mix stay exact (rebalance by construction, never net-new count). A
       frame front may EXPAND (s108 expand) rearward, consuming its first
       rear slot — the occupant gets a teardown arc that COMPLETES before
       the annex breaks ground (the ruins-gate discipline holds). */
var FILL_CVF_FRAC         = 0.35;         // seeded fraction of non-tent, non-core 1849-wave fronts that BEGIN as canvas
var FILL_TENT_HARDEN_FRAC = 0.30;         // seeded fraction of residential-band 1849-wave TENT fronts that HARDEN into frame on the band interval (fill-density-model §4: the tent->frame single-year replacement cycle, peak 1849 — the s106d core arc at band pace; the rest persist as canvas through the window)
var FILL_RES_CONV_MIN_D   = 60;           // residential-band canvas->final hardening: earliest (~2 months after ITS spawn)
var FILL_RES_CONV_MAX_D   = 240;          // ...latest (~8 months) — the residential band sheds canvas SLOWER than the core's 60-150d
var FILL_REAR_FRAC        = 0.40;         // cap: rear-slot placements <= ceil(frac x shanty-bucket placements) (streaming, like the core-canvas cap; ics26 raised 0.25 -> 0.40 — denser rear courts carry the RES-4 interior texture, spread town-wide by the round-robin cursor)
var FILL_REAR_GAP_M       = 1.5;          // front-occupant -> rear-occupant spacing inside a lot (tandem pitch)
var FILL_REAR_CAP_WIDE_M  = 5.0;          // a front at least this wide anchors 2 rear slots; narrower lots host 1
var FILL_REAR_MIN_STAND_D = 20;           // a consumed rear tent must have STOOD at least this long before its teardown
var FILL_EXP_FRAC         = 0.60;         // seeded fraction of eligible lots (frame-formed front + rear occupant) whose front expands — eligibility itself is scarce (the front must STAND AS FRAME first), so the fraction runs high to keep the mechanic visible
var FILL_EXP_DUR_D        = 25;           // annex construction duration (frame-annex scale work)
var FILL_EXP_START_ISO    = "1849-10-01"; // expansion window (seeded within): consolidation follows the boom's fall money
var FILL_EXP_END_ISO      = "1850-03-01";
/* =====================================================================
   ics26 (ICS-26) tunables — MEASURED BLOCK-COVERAGE TARGETS (RES-4,
   research/block-coverage-1849-53.md §6). The Dec-1849 fill is tuned to the
   measured per-band structure-coverage curve, via a HOTSPOT-FIELD model
   (operator design): density grows around MULTIPLE seeded nuclei — the plaza
   is one hotspot, each ACTIVE wharf-foot corridor is another (the RES-4
   Sansome-Battery finding: density followed the wharves, not just the
   plaza) — never one city-center radial gradient. A block's coverage TARGET
   is drawn from the MEASURED in-band spread (core 30-65% etc.), seeded per
   block, so blocks vary like the real chart; the audit gates the per-band
   AGGREGATE + spread, never uniform values.

   THE FIELD: each hotspot h radiates a decay field with its OWN reach
   (coreR/midR — a wharf-foot colony is real but tighter than the plaza's);
   a point's grade is the SUPERPOSITION (the strongest grade any nearby
   hotspot grants — "bacterial colony" fronts merging), and demand order is
   the normalized field coordinate min_h d(p,h)/coreR_h. Radii calibrated in
   metres against RES-4's block-distance bands. */
var FILL_COV_CORE_M    = 240;    // plaza hotspot: core-grade reach (RES-4 CORE = plaza-facing + ~1-2 blocks)
var FILL_COV_MID_M     = 500;    // plaza hotspot: mid-grade reach (RES-4 MID ~2-4 blocks, ~250-500 m); beyond -> fringe
var FILL_WF_CORE_M     = 170;    // wharf-foot hotspot: core-grade reach (the corridor-adjacent blocks — the M5 exception; = the audit's WATERFRONT radius)
var FILL_WF_MID_M      = 280;    // wharf-foot hotspot: mid-grade reach (the colony shoulders, ~1 block beyond the corridor)
/* per-band coverage RANGES (RES-4 measured spreads, Dec-1849 discounted) +
   the skew that pins the sampled band MEAN at the recommended target:
   E[lo + (hi-lo)*u^k] = lo + (hi-lo)/(k+1)  ->  core [0.30,0.65] k=6 => mean
   0.35; mid [0.07,0.17] k=1 => 0.12; fringe [0.01,0.05] k=1 => 0.03. */
var FILL_COV_RANGE = { core: [0.30, 0.65], mid: [0.07, 0.17], fringe: [0.01, 0.05] };
var FILL_COV_SKEW  = { core: 6, mid: 1, fringe: 1 };
var FILL_COV_TOL   = { core: 0.08, mid: 0.05, fringe: 0.02 };   // audit tolerance (RES-4 confidence tier C)
var FILL_WF_RADIUS_M   = 170;    // audit band: a non-core block whose centroid is within this of an ACTIVE wharf foot is WATERFRONT-FILL (core-grade expected — the M5 exception; = FILL_WF_CORE_M)
var FILL_COV_BUILD_MIN = 0.5;    // a block under half BUILDABLE at the wave start is not a coverage subject — its ground belongs to the cove/beach/hillside, and its density arrives with the 1850-51 fill (RES-4: M5 itself is measured on MADE land)
/* size grading (fill-density-model §7: dense ground weights medium/large):
   catalog-code pick weight *= (footprint_ft2 / 300)^bias per grade, and the
   chosen footprint DEPTH is scaled into the block (RES-4: perimeter-first,
   the built rim deepens where demand is hottest), capped absolutely. */
var FILL_SIZE_BIAS   = { core: 1.4, mid: 0.3, fringe: -0.8 };
var FILL_DEPTH_SCALE = { core: 1.60, mid: 1.20, fringe: 1.0 };
var FILL_WIDTH_SCALE = { core: 1.25, mid: 1.10, fringe: 1.0 };
var FILL_SCALE_DEPTH_MAX_M = 16.5;  // ~54 ft — an annexed-depth cap so scaled fronts never reach mid-block ROWs / rear courts
var FILL_SCALE_WIDTH_MAX_M = 20.0;
/* backyard texture — property-line fences (activates the placement.fences
   stub): a seeded fraction of RESIDENTIAL-band wave lots ring their yard
   with placeholder post-and-rail runs, appearing a seeded lag after the
   lot's front building. */
var FILL_FENCE_FRAC      = 0.35;
var FILL_FENCE_LAG_MIN_D = 10, FILL_FENCE_LAG_MAX_D = 45;
var FILL_FENCE_MARGIN_M  = 0.6;     // rail offset beyond the widest occupant's half-width
var FILL_FENCE_POST_M    = 2.5;     // post pitch along a rail

/* the ACTIVE hotspots at `day`: the plaza + every pier whose construction
   checkpoint is reached (pierEdgesAt), each contributing its FOOT (innerEnd —
   where the wharf meets the town: the corridor head Sansome-Battery densified
   around). Derived from the pier record at query time — no hardcoded coords. */
function fillHotspotsAt(day){
  var H = [{ id: "plaza", x: PLAZA_CENTER.x, z: PLAZA_CENTER.z, coreR: FILL_COV_CORE_M, midR: FILL_COV_MID_M }];
  var piers = (typeof PIERS_RUNTIME !== "undefined") ? PIERS_RUNTIME : [];
  for(var i = 0; i < piers.length; i++){
    var e = (typeof pierEdgesAt === "function") ? pierEdgesAt(piers[i], day) : null;
    if(e && e.active && e.innerEnd) H.push({ id: piers[i].id, x: e.innerEnd.x, z: e.innerEnd.z, coreR: FILL_WF_CORE_M, midR: FILL_WF_MID_M });
  }
  return H;
}
/* the field's normalized level-set coordinate — min_h d(p,h)/coreR_h (demand
   order: 1.0 = the edge of some hotspot's core-grade reach). */
function fillEffDist(x, z, hots){
  var best = Infinity;
  for(var i = 0; i < hots.length; i++){
    var h = hots[i], d = Math.hypot(x - h.x, z - h.z) / (h.coreR || 1);
    if(d < best) best = d;
  }
  return best;
}
/* the SUPERPOSED grade — the strongest grade ANY hotspot grants the point. */
function fillCovGradeAt(x, z, hots){
  var g = 2;
  for(var i = 0; i < hots.length; i++){
    var h = hots[i], d = Math.hypot(x - h.x, z - h.z);
    if(d <= h.coreR) return "core";
    if(d <= h.midR && g > 1) g = 1;
  }
  return g === 1 ? "mid" : "fringe";
}

/* THE COVERAGE BLOCK TABLE — every fifty-vara block that is a coverage
   subject at the wave cap: born, not the plaza common, majority-dry at the
   wave start. Carries the block's world centroid, area, dry fraction
   (8x6 interior grid vs isDryLand at the wave start), the audit BAND
   (RES-4 measurement bands: plaza distance + the waterfront exception) and
   the generator GRADE (hotspot field). Cached per FILL_QA.version. */
var _fillCovBlocks = null, _fillCovBlocksVer = -1;
function fillCovBlocks(){
  if(_fillCovBlocks && _fillCovBlocksVer === FILL_QA.version) return _fillCovBlocks;
  var capDay = eventDateToSimDay(FILL_1849_CAP_ISO), startDay = eventDateToSimDay(FILL_1849_START_ISO);
  var hots = fillHotspotsAt(capDay);
  var out = [], B = (typeof GROUND_PLAN !== "undefined") ? GROUND_PLAN.blocks : [];
  for(var bi = 0; bi < B.length; bi++){
    var b = B[bi];
    if(b.district !== "fifty-vara" || b.publicReserve) continue;
    if(b.birth > capDay) continue;
    var wc = gridToWorldAt((b.uLo + b.uHi) / 2, (b.vLo + b.vHi) / 2, GRID_ROT_BASE);
    /* BUILDABLE fraction — a 10x8 interior sample grid at the wave start,
       against the SAME ground laws the structure placement row enforces
       (LAW_TABLES.structure minY + slopeMaxPct + dry land): coverage is
       measured over ground a structure can lawfully stand on — the beach
       band and the steep hillside are not coverage subjects at this window
       (their density arrives with the 1850-51 fill / grading; RES-4's M5
       waterfront blocks are themselves measured on MADE land). */
    var sLaw = (typeof LAW_TABLES !== "undefined" && LAW_TABLES.structure) ? LAW_TABLES.structure : { minY: 1, slopeMaxPct: 15 };
    var dryN = 0, buildN = 0, tot = 0;
    for(var a = 0; a < 10; a++) for(var c = 0; c < 8; c++){
      var u = b.uLo + b.extU * (a + 0.5) / 10, v = b.vLo + b.extV * (c + 0.5) / 8;
      var w = gridToWorldAt(u, v, GRID_ROT_BASE); tot++;
      var dry = (typeof isDryLand !== "function" || isDryLand(w.x, w.z, startDay));
      if(dry){
        dryN++;
        if(terrainHeight(w.x, w.z) > sLaw.minY &&
           (typeof terrainSlopePct !== "function" || terrainSlopePct(w.x, w.z) <= sLaw.slopeMaxPct)) buildN++;
      }
    }
    var dryFrac = tot ? dryN / tot : 0, buildFrac = tot ? buildN / tot : 0;
    var plazaD = Math.hypot(wc.x - PLAZA_CENTER.x, wc.z - PLAZA_CENTER.z);
    var wharfD = Infinity;
    for(var hi = 1; hi < hots.length; hi++){                 // hots[0] is the plaza
      var hd = Math.hypot(wc.x - hots[hi].x, wc.z - hots[hi].z);
      if(hd < wharfD) wharfD = hd;
    }
    var band = plazaD <= FILL_COV_CORE_M ? "core"
             : (wharfD <= FILL_WF_RADIUS_M ? "waterfront"
             : (plazaD <= FILL_COV_MID_M ? "mid" : "fringe"));
    out.push({ key: b.key, uLo: b.uLo, uHi: b.uHi, vLo: b.vLo, vHi: b.vHi,
               cx: wc.x, cz: wc.z, areaM2: b.extU * b.extV,
               dryFrac: +dryFrac.toFixed(3), buildFrac: +buildFrac.toFixed(3),
               eligible: buildFrac >= FILL_COV_BUILD_MIN,
               plazaD: +plazaD.toFixed(1), wharfD: isFinite(wharfD) ? +wharfD.toFixed(1) : null,
               band: band, dEff: +fillEffDist(wc.x, wc.z, hots).toFixed(3),
               grade: fillCovGradeAt(wc.x, wc.z, hots) });
  }
  _fillCovBlocks = out; _fillCovBlocksVer = FILL_QA.version;
  return out;
}
/* which coverage block a world point sits in (uv rect test; null off-plat). */
function fillCovBlockAt(blocks, x, z){
  var g = cadWorldToGridAt(x, z, GRID_ROT_BASE);
...
/* THE SPAWNER LEDGER — per-block sampled coverage targets + a running placed-
   area account. target = lo + (hi-lo) * hash^skew of the block's GRADE range
   (seeded per block — the measured in-band spread, never one painted mean).
   Seeded up-front with everything already standing at the cap (landmarks +
   the v1 village), so the wave budgets the REMAINDER. */
function fillCovLedger(capDay){
  var blocks = fillCovBlocks(), byKey = {};
  for(var i = 0; i < blocks.length; i++){
    var b = blocks[i], r = FILL_COV_RANGE[b.grade], k = FILL_COV_SKEW[b.grade];
    var u = lcHash("COV|" + b.key, "cov");
    var target = r[0] + (r[1] - r[0]) * Math.pow(u, k);
    byKey[b.key] = { b: b, target: target, budgetM2: target * b.areaM2 * Math.max(b.buildFrac, 0.05), placedM2: 0 };
  }
  function cell(x, z){ var b = fillCovBlockAt(blocks, x, z); return b ? byKey[b.key] : null; }
  return {
    blocks: blocks, byKey: byKey,
    seed: function(cx, cz, areaM2){ var c = cell(cx, cz); if(c) c.placedM2 += areaM2; },
    gradeAt: function(x, z){ var c = cell(x, z); return c ? c.b.grade : "fringe"; },
    allow: function(x, z, areaM2){ var c = cell(x, z); return !c || c.placedM2 < c.budgetM2; },
    book: function(x, z, areaM2){ var c = cell(x, z); if(c) c.placedM2 += areaM2; },
    summary: function(){
      var s = {};
      for(var k2 in byKey){ var c = byKey[k2];
        var band = c.b.band, g = s[band] || (s[band] = { blocks: 0, targetSum: 0, placedSum: 0, budgetSum: 0 });
        g.blocks++; g.targetSum += c.target; g.placedSum += c.placedM2; g.budgetSum += c.budgetM2;
      }
      return s;
    }
  };
}
/* THE MEASURE (shared by the blockCoverage audit + the census hook —
   measurement law: one measure, re-derived from the world, never the
   spawner's ledger). Coverage of an eligible block at `day` = the summed
   footprint area of every STANDING structure whose centroid lies in the
   block (derived fill in a physical phase + the landmark set — the drawn
   OBBs, expansion included) / the block's DRY area. */
function fillBlockCoverageMeasure(day){
  var blocks = fillCovBlocks();
  var acc = {};
  for(var i = 0; i < blocks.length; i++) acc[blocks[i].key] = 0;
  function add(cx, cz, w, d){
    var b = fillCovBlockAt(blocks, cx, cz);
    if(b) acc[b.key] += w * d;
  }
  deriveFillSet(day).forEach(function(e){
    if(e.phase === "absent" || e.phase === "gone" || e.phase === "cleared" ||
       e.phase === "ruins" || e.phase === "clearing") return;
    add(e.cx, e.cz, e.w, e.d);
  });
  deriveLandmarkSet(day).forEach(function(L){
    if(L.phase === "absent" || L.phase === "gone" || L.phase === "cleared" ||
       L.phase === "ruins" || L.phase === "clearing") return;
    add(L.cx, L.cz, L.w, L.d);
  });
  var rows = [], bands = {};
  for(var j = 0; j < blocks.length; j++){
    var b = blocks[j];
    if(!b.eligible) continue;
    var cov = acc[b.key] / (b.areaM2 * b.buildFrac);
    rows.push({ key: b.key, band: b.band, grade: b.grade, cov: +cov.toFixed(4),
                x: +b.cx.toFixed(1), z: +b.cz.toFixed(1),
                dryFrac: b.dryFrac, buildFrac: b.buildFrac, plazaD: b.plazaD, wharfD: b.wharfD, areaM2: +acc[b.key].toFixed(0) });
    var g = bands[b.band] || (bands[b.band] = { n: 0, sum: 0, min: Infinity, max: -Infinity });
    g.n++; g.sum += cov; if(cov < g.min) g.min = cov; if(cov > g.max) g.max = cov;
  }
  var perBand = {};
  Object.keys(bands).forEach(function(k){
    var g = bands[k];
    perBand[k] = { n: g.n, mean: +(g.sum / g.n).toFixed(4), min: +g.min.toFixed(4), max: +g.max.toFixed(4) };
  });
  return { day: day, blocks: rows, perBand: perBand };
}
/* the canvas-first arc numbers for ONE entry — a pure lcHash function of the
   entry identity, used by BOTH fillLifecycleOf (the events) and the expansion
   post-pass in buildFillMaster (front-readiness), so the two never disagree
   (measurement law). */
function fillCvfArc(e){
  var id = e.code + "#" + e.rank;
  var tdStart = e.appearDay + FILL_RES_CONV_MIN_D + lcHash(id, "cvfday") * (FILL_RES_CONV_MAX_D - FILL_RES_CONV_MIN_D);
  var tdOn = tdStart + LC_TEARDOWN_DAYS;
  var rbStart = tdOn + 2;
  var lead = 14 + Math.floor(lcHash(id, "cvfrb") * 10);
  return { tdStart: tdStart, tdOn: tdOn, rbStart: rbStart, rbOn: rbStart + lead, lead: lead };
}
/* the s108 ~30% shanty UPGRADE-WAVE arc for one entry (teardown-for-
   replacement -> FRAME rebuild, Mar-Aug 1849) — the EXACT law fillLifecycleOf
   has carried since s108, factored to a pure helper because the s106e
   expansion post-pass must know whether a lot's front STANDS AS FRAME by the
   expansion window (an upgraded shanty front is a frame building thereafter).
   Returns null when the entry takes no upgrade arc. Same lcHash salts, same
   thresholds — the compiled lifecycles are event-identical to s106d. */
function fillUpgradeArc(e){
  if(e.cat !== "shanty" || e.coreCanvas || e.rearOf != null || e.canvasFirst || e.tentHarden) return null;
  var id = e.code + "#" + e.rank;
  if(lcHash(id, "upg") >= 0.30) return null;
  var t0 = eventDateToSimDay("1849-03-01") + Math.floor(lcHash(id, "upgday") * 180);   // Mar-Aug 1849
  if(t0 <= e.appearDay + 30) return null;
  var tOn = t0 + LC_TEARDOWN_DAYS, rbStart = tOn + 2;
  return { t0: t0, tOn: tOn, rbStart: rbStart, rbOn: rbStart + 21 };
}
/* s106d — ONE core-ground measure shared by the wave spawner and the
   zoneCoreCanvas audit (measurement law): a point is CORE-BLOCK ground when
   the cadastre's GOVERNING zone there is the era-grown commercial-core. */
function fillCoreBlockAt(x, z, day){
  return typeof landUseZoneAt === "function" && landUseZoneAt(x, z, day) === "commercial-core";
}
...
function buildFillMaster(){
  ...
  /* ===================================================================
     ...
     EVERY existing gate holds (the operator's law):
       ...
       • zone law — evaluated at the WAVE CAP date (the era-grown
         ...
         the window): the core admits frame, so tents/shanties thread
         the RESIDENTIAL band while frame densifies the core blocks —
         EXCEPT the s106d dated, bounded core-canvas exception (a capped
         ~12% canvas fraction of core-block wave placements rides the
         zone gate's boom-window coreCanvasPermit; see the sub-walk
         below + buildings.zoneCoreCanvas);
  ...
  /* ics26 (ICS-26) — THE HOTSPOT-FIELD COVERAGE LEDGER. Per-block coverage
     targets sampled from the RES-4 measured in-band spreads (fillCovLedger),
     seeded with everything ALREADY standing at the cap (the v1 village + the
     landmark registry) so the wave budgets the remainder. Demand order is the
     FIELD's effective distance (plaza + active wharf-foot hotspots — the
     Sansome-Battery finding), not a single plaza radius. QA knob
     ignoreCoverageTargets reverts to the flat plaza-radial walk (the
     blockCoverage fail-before). */
  var cov26 = FILL_QA.ignoreCoverageTargets ? null : fillCovLedger(waveCap);
  var hots26 = fillHotspotsAt(waveCap);
  if(cov26){
    for(var sv = 0; sv < v1Len; sv++) cov26.seed(master[sv].cx, master[sv].cz, master[sv].w * master[sv].d);
    deriveLandmarkSet(waveCap).forEach(function(L){
      if(L.phase === "absent" || L.phase === "gone" || L.phase === "cleared") return;
      cov26.seed(L.cx, L.cz, L.w * L.d);
    });
  }
  /* ics26 — WAVE PACKING IS OBB-EXACT. The v1 loop's circle-conservative
     inter-fill check wasted whole frontage runs once footprints grew (a
     16x9 m store's bounding circle "overlaps" neighbours its BOX clears by
     metres). The wave proves detachment on the DRAWN BOX (SAT, the same
     measure fillFootprintsOBB gates) behind a circle broadphase; the v1
     build keeps its historic circle check untouched (byte-identical). */
  var placedQ = [];
  for(var pq0 = 0; pq0 < v1Len; pq0++){
    var pe0 = master[pq0];
    placedQ.push({ x: pe0.cx, z: pe0.cz, r: Math.hypot(pe0.w, pe0.d) / 2,
                   q: fillPoseQuad(pe0.cx, pe0.cz, pe0.w, pe0.d, pe0.yaw) });
  }
  function waveOverlaps(cx, cz, r, quad){
    for(var i = 0; i < placedQ.length; i++){
      var p = placedQ[i], dx = cx - p.x, dz = cz - p.z, md = r + p.r;
      if(dx * dx + dz * dz >= md * md) continue;
      if(fillQuadPen(quad, p.q) > 0) return true;
    }
    return false;
  }
  for(var s2 = 0; s2 < slots2.length; s2++){
    slots2[s2].order = cov26
      ? fillEffDist(slots2[s2].cx, slots2[s2].cz, hots26) + (rng2() - 0.5) * 0.08          // normalized field coordinate + jitter
      : Math.hypot(slots2[s2].cx - PLAZA_CENTER.x, slots2[s2].cz - PLAZA_CENTER.z) + (rng2() - 0.5) * 10;
  }
  slots2.sort(function(a, b){ return a.order - b.order; });
  var counts2 = { shanty: 0, frame: 0, adobe: 0 }, waveN = 0;
  var rej2 = { edgeCap: 0, camp: 0, water: 0, zone: 0, law: 0, row: 0, reserved: 0, overlap: 0, noType: 0, cov: 0 };
  ...
  /* s106d (ICS-24) — THE BOOM-WINDOW CORE-CANVAS EXCEPTION (operator ruling;
     careful by design). The s106c law stood absolute: the commercial front is
     never tent ground, so the whole shanty/tent bucket threaded the
     residential band. The record's core carried SOME canvas among the frame
     through the boom (the marquee authored case: the El Dorado's canvas
     phase ON the plaza) — so the wave now routes a LIMITED canvas fraction
     of its shanty bucket onto CORE blocks under the zone gate's dated
     coreCanvasPermit (core/08 CAD_CORE_CANVAS_*): the streaming cap
     coreCanvas+1 <= ceil(frac x (coreWave+1)) guarantees the final bound
     coreCanvas <= ceil(frac x coreWave) at every step (the zoneCoreCanvas
     audit re-measures it on the master with the SAME fillCoreBlockAt ground
     measure). NOT open season: outside the window the gate voids the permit
     (the existing law), and the placement is a RESHUFFLE of the shanty
     bucket (counts2.shanty carries it — the 55/45 tent-first mix is exact,
     never net-new count). A core tent takes the shanty turn INSTEAD of a
     residential slot; every other wave gate below (camp / dry-land / law
     rows / ROW / reserved / spacing / edge budget) holds identically. The
     dedicated coreCursor walks core slots plaza-ward and burns only genuine
     failures, so the quota reopening as frame densifies the core keeps
     admitting canvas throughout the ramp. QA: uncapCoreCanvas drops the
     fraction cap (fail-before for zoneCoreCanvas, probe-s106d). */
  var coreWaveN = 0, coreCanvasN = 0, coreCursor = 0, coreCanvasDone = false;
  /* s106e (ICS-25) — THE LOT/SLOT REGISTRY. Every non-core wave FRONT
     placement registers a LOT: the front occupant + 1-2 rear tandem slot
     positions reaching into the block (capacity by front width — the size
     class the catalog footprint implies). A rear slot is filled by routing
     a shanty-bucket turn to it (the sub-walk below), so rear tents are a
     RESHUFFLE of the wave's shanty share, never net-new count. depthUsed
     tracks how deep the lot's tandem column has grown; slotBad memoizes a
     slot position whose gates can never pass (static geometry — a failed
     slot never un-fails). */
  var lots = [], rearN = 0, rearCursor = 0;   // ics26: persistent round-robin cursor — rear slots spread town-wide (the s106e one-line cursor policy), never re-packing the plaza-nearest lots first
  while(waveN < need2){
    var cat2 = wavePickCat();
    if(!cat2) break;                                      // every category out of legal ground
    if(cat2 === "shanty" && !coreCanvasDone && !FILL_QA.ignoreInTownGates &&
       (FILL_QA.uncapCoreCanvas || coreCanvasN + 1 <= Math.ceil(CAD_CORE_CANVAS_FRAC * (coreWaveN + 1)))){
      var ct = fillPickCode("shanty", waveCap, rng2, "1849", true);   // CANVAS only — the exception never admits the scrap tier
      var placedCore = false, ci = coreCursor;
      for(; ct && ci < slots2.length; ci++){
        var cs = slots2[ci];
        if(!fillCoreBlockAt(cs.cx, cs.cz, waveCap)) continue;         // core-block ground only (probe point, pre-rng; re-proven at the final pose below)
        var cwM = ct.footprint.w * FILL_FT_M, cdM = ct.footprint.d * FILL_FT_M;
        var cBuilt = edgeBuilt[cs.edgeId] || 0;
        if((cBuilt + cwM) / cs.edgeLen > FILL_EDGE_CAP_1849) continue;
        var cJit = (rng2() - 0.5) * Math.min(FILL_SLOT_STEP_M * 0.5, cs.edgeLen * 0.15);
        var cSet = FILL_SETBACK_M + cdM / 2;
        var ccx = cs.ex + cs.tx * cJit + cs.inx * cSet;
        var ccz = cs.ez + cs.tz * cJit + cs.inz * cSet;
        if(!fillCoreBlockAt(ccx, ccz, waveCap)) continue;             // the posed centre must be core ground too (the audit's measure)
        if(cov26 && !cov26.allow(ccx, ccz, cwM * cdM)) continue;      // ics26: the block is at its sampled coverage target
        var crB = Math.hypot(cwM, cdM) / 2;
        if(typeof encampmentZoneAt === "function" && encampmentZoneAt(ccx, ccz, waveCap)) continue;
        if(typeof isDryLand === "function" && !isDryLand(ccx, ccz, waveStart)) continue;
        var ccp = canPlace("tent", { x: ccx, z: ccz, yaw: cs.yaw, footprint: crB, footprintW: cwM, footprintD: cdM },
                           { day: waveCap, zoneClass: "tent", coreCanvasPermit: true });
        if(!ccp.ok) continue;
        var cQuad26 = fillPoseQuad(ccx, ccz, cwM, cdM, cs.yaw);
        if(fillQuadCrossesRow(cQuad26, ccx, ccz)) continue;
        var cResBad = false;
        for(var cpi = 0; cpi < PLACEMENT_INDEX.length; cpi++){
          var cq = PLACEMENT_INDEX[cpi], cdx = ccx - cq.x, cdz = ccz - cq.z, cmd = crB + cq.r;
          if(cdx*cdx + cdz*cdz < cmd*cmd){ cResBad = true; break; }
        }
        if(cResBad) continue;
        if(waveOverlaps(ccx, ccz, crB, cQuad26)) continue;
        counts2.shanty++; waveN++; coreWaveN++; coreCanvasN++;
        if(cov26) cov26.book(ccx, ccz, cwM * cdM);
        edgeBuilt[cs.edgeId] = cBuilt + cwM + FILL_GAP_M;
        placedQ.push({ x: ccx, z: ccz, r: crB, q: cQuad26 });
        master.push({ code: ct.code, cat: "shanty", material: ct.material, subtype: ct.subtype,
                      cx: ccx, cz: ccz, w: cwM, d: cdM, yaw: cs.yaw, nx: cs.outx, nz: cs.outz,
                      storyRange: ct.storyRange || [1, 1], edgeId: cs.edgeId, edgeLen: cs.edgeLen,
                      overWater: !!ct.overWater, stilted: !!ct.stilted,
                      rank: master.length, appearDay: waveCap, wave: 1849, coreCanvas: 1 });
        placedCore = true; ci++;
        break;
      }
      coreCursor = ci;
      if(ci >= slots2.length) coreCanvasDone = true;      // the core slot supply is spent — reported, never silently hidden
      if(placedCore) continue;                            // this shanty turn went to the core under the window cap
    }
    /* s106e (ICS-25) — THE REAR-SLOT SUB-WALK. A shanty turn the core did
       not take may be routed to a REAR slot of an already-placed lot under
       the streaming cap rearN+1 <= ceil(FILL_REAR_FRAC x (shanty+1)) — the
       same discipline as the core-canvas cap. The occupant is a TENT (the
       rear-court texture; canvas, never the scrap tier), posed in tandem
       behind the lot's front occupant, same yaw (facing the lot's street),
       a seeded lateral tuck within the front's width band. EVERY wave gate
       holds: no core ground (the rear-tent texture is residential; the core
       has its own capped canvas law), no documented-camp ground, dry land at
       the wave start, the canPlace tent law rows (corner-level), no mid-block
       ROW crossing, no reserved landmark ground, detached vs ALL fill. No
       frontage budget is booked (a rear tent stands OFF the street edge; its
       edgeId carries a #rear suffix so the openness measures never conflate
       it with frontage). QA: ignoreSlotGates poses the tent on the STREET
       side of the front and skips the gates, so rear tents straddle rights-
       of-way and the lotSlots packing clause goes RED (fail-before). */
    if(cat2 === "shanty" && lots.length &&
       rearN + 1 <= Math.ceil(FILL_REAR_FRAC * (counts2.shanty + 1))){
      var rt = fillPickCode("shanty", waveCap, rng2, "1849", true);   // rear occupants are CANVAS
      var placedRear = false;
      for(var li0 = 0; rt && li0 < lots.length && !placedRear; li0++){
        var li = (rearCursor + li0) % lots.length;                    // ics26: round-robin over ALL lots
        var lot = lots[li];
        if(lot.rearUsed >= lot.rearCap) continue;
        var slotK = lot.rearUsed + 1;
        if(lot.slotBad[slotK]) continue;
        var fe = master[lot.frontIdx];
        var rw = rt.footprint.w * FILL_FT_M, rd = rt.footprint.d * FILL_FT_M;
        /* tandem pose: behind the column's current depth; lateral tuck seeded
           within the front's width band (pure hash — no stream draw, so a
           failed slot costs zero dice and stays deterministic). Knob-on
           mirrors the pose to the STREET side (the fail-before offense). */
        var rDir = FILL_QA.ignoreSlotGates ? 1 : -1;      // -n = into the block; +n = onto the street (QA)
        var rBack = lot.depthUsed + FILL_REAR_GAP_M + rd / 2;
        var rLat  = (lcHash("LOT#" + fe.rank, "rlat" + slotK) - 0.5) * Math.max(0, fe.w - rw);
        var rtx = fe.nz, rtz = -fe.nx;                    // width axis (same frame as fillPoseQuad)
        var rcx = fe.cx + rDir * fe.nx * rBack + rtx * rLat;
        var rcz = fe.cz + rDir * fe.nz * rBack + rtz * rLat;
        var rB = Math.hypot(rw, rd) / 2, rBad = false;
        var rQuad26 = fillPoseQuad(rcx, rcz, rw, rd, fe.yaw);
        if(cov26 && !cov26.allow(rcx, rcz, rw * rd)){ lot.slotBad[slotK] = true; continue; }   // ics26: block at its coverage target (budgets only ever fill — a static fail)
        if(!FILL_QA.ignoreSlotGates){
          if(fillCoreBlockAt(rcx, rcz, waveCap)) rBad = true;                                        // rear tents are residential texture
          else if(typeof encampmentZoneAt === "function" && encampmentZoneAt(rcx, rcz, waveCap)) rBad = true;
          else if(typeof isDryLand === "function" && !isDryLand(rcx, rcz, waveStart)) rBad = true;
          else {
            var rcp = canPlace("tent", { x: rcx, z: rcz, yaw: fe.yaw, footprint: rB, footprintW: rw, footprintD: rd },
                               { day: waveCap, zoneClass: "tent" });
            if(!rcp.ok) rBad = true;
            else if(fillQuadCrossesRow(rQuad26, rcx, rcz)) rBad = true;
            else {
              for(var rpi = 0; rpi < PLACEMENT_INDEX.length; rpi++){
                var rq = PLACEMENT_INDEX[rpi], rqx = rcx - rq.x, rqz = rcz - rq.z, rqm = rB + rq.r;
                if(rqx*rqx + rqz*rqz < rqm*rqm){ rBad = true; break; }
              }
            }
          }
          if(rBad){ lot.slotBad[slotK] = true; continue; }   // static geometry — this slot never un-fails
        }
        if(waveOverlaps(rcx, rcz, rB, rQuad26)){ lot.slotBad[slotK] = true; continue; }
        counts2.shanty++; waveN++; rearN++;
        if(cov26) cov26.book(rcx, rcz, rw * rd);
        placedQ.push({ x: rcx, z: rcz, r: rB, q: rQuad26 });
        master.push({ code: rt.code, cat: "shanty", material: rt.material, subtype: rt.subtype,
                      cx: rcx, cz: rcz, w: rw, d: rd, yaw: fe.yaw, nx: fe.nx, nz: fe.nz,
                      storyRange: rt.storyRange || [1, 1], edgeId: fe.edgeId + "#rear", edgeLen: fe.edgeLen,
                      overWater: !!rt.overWater, stilted: !!rt.stilted,
                      rank: master.length, appearDay: waveCap, wave: 1849,
                      rearOf: fe.rank, slot: slotK });
        lot.rearUsed++; lot.rearIdxs.push(master.length - 1);
        lot.depthUsed = rBack + rd / 2;
        placedRear = true;
        rearCursor = (li + 1) % lots.length;              // ics26: the next rear turn continues town-wide from here
      }
      if(placedRear) continue;                            // this shanty turn seated a rear-court tent
    }
    var placedOne = false, idx2 = cursors2[cat2];
    for(; idx2 < slots2.length; idx2++){
      var slot2 = slots2[idx2];
      /* ics26 — SIZE GRADING by the slot's field grade (fill-density-model
         §7): the code pick weights medium/large on core-grade ground and
         small on the fringe (areaBias), and the chosen NON-TENT footprint is
         depth-scaled into the block (RES-4's perimeter-first, deepened rim),
         capped absolutely. The scaled dims feed EVERY placement gate below
         (edge budget / canPlace / ROW / overlap), so legality is proven on
         the drawn box. */
      var g26 = cov26 ? cov26.gradeAt(slot2.cx, slot2.cz) : null;
      var t2 = fillPickCode(cat2, waveCap, rng2, "1849", false, g26 ? FILL_SIZE_BIAS[g26] : 0);
      if(!t2){ rej2.noType++; idx2 = slots2.length; break; }
      var wM2 = t2.footprint.w * FILL_FT_M, dM2 = t2.footprint.d * FILL_FT_M;
      if(g26 && t2.subtype !== "tent"){
        wM2 = Math.min(wM2 * FILL_WIDTH_SCALE[g26], FILL_SCALE_WIDTH_MAX_M);
        dM2 = Math.min(dM2 * FILL_DEPTH_SCALE[g26], FILL_SCALE_DEPTH_MAX_M);
      }
      ...
      var quad26 = fillPoseQuad(cx2, cz2, wM2, dM2, slot2.yaw);
      if(cov26 && !cov26.allow(cx2, cz2, wM2 * dM2)){ rej2.cov++; continue; }   // ics26: the block sits at its sampled coverage target
      if(!FILL_QA.ignoreInTownGates){
        ...
        if(fillQuadCrossesRow(quad26, cx2, cz2)){ rej2.row++; continue; }
        var resBad2 = false;
      ...
      if(waveOverlaps(cx2, cz2, rB2, quad26)){ rej2.overlap++; continue; }
      counts2[cat2]++; waveN++;
      if(cov26) cov26.book(cx2, cz2, wM2 * dM2);
      var onCore2 = fillCoreBlockAt(cx2, cz2, waveCap);
      if(onCore2) coreWaveN++;   // s106d: core-block placements feed the streaming canvas cap (same ground measure as the audit)
      edgeBuilt[slot2.edgeId] = built2 + wM2 + FILL_GAP_M;
      placedQ.push({ x: cx2, z: cz2, r: rB2, q: quad26 });
      master.push({ code: t2.code, cat: cat2, material: t2.material, subtype: t2.subtype,
                    ...
                    overWater: !!t2.overWater, stilted: !!t2.stilted, covGrade: g26 || null,
                    rank: master.length, appearDay: waveCap, wave: 1849 });
      /* s106e — register the LOT this front anchors (non-core ground only:
         the rear-tent texture is residential; core blocks keep the s106d
         capped canvas law). Capacity by size class: a wide front (the
         medium-class 18-ft cottage tier) anchors 2 tandem rear slots, a
         narrow one (small class / tents) anchors 1. depthUsed starts at the
         front's own half-depth — the tandem column grows from its rear face. */
      if(!onCore2){
        lots.push({ frontIdx: master.length - 1,
                    rearCap: wM2 >= FILL_REAR_CAP_WIDE_M ? 2 : 1,
                    rearUsed: 0, rearIdxs: [], slotBad: {}, depthUsed: dM2 / 2 });
      }
      placedOne = true; idx2++;
      break;
    }
  ...
  /* ===================================================================
     s106e (ICS-25) — CANVAS-FIRST MARKING (mechanic 1). A seeded fraction
     of the wave's RESIDENTIAL-band FRONT placements (never a tent — it IS
     canvas; never a rear occupant — rear slots have their own stay-canvas
     law; never core ground — the core keeps the s106d capped exception)
     BEGIN as canvas and harden into their final catalog form. The decision
     is a pure lcHash of the entry identity (zero stream draws — the wave
     layout above is untouched by the marking) and the fields live on the
     master entry so fillLifecycleOf and the expansion pass below read ONE
     record. This is a PATH change only: rank/cat/footprint (the census +
     mix + placement facts) are already fixed above. ===================== */
  var cvfN = 0, thN = 0;
  for(var mi = v1Len; mi < master.length; mi++){
    var me = master[mi];
    if(me.coreCanvas || me.rearOf != null) continue;
    if(fillCoreBlockAt(me.cx, me.cz, waveCap)) continue;
    if(me.subtype === "tent"){
      /* a TENT front already IS canvas — its canvas-first arc is the OTHER
         side of the same law: a seeded fraction HARDEN into FRAME on the band
         interval (fill-density-model §4's tent->frame single-year cycle; the
         s106d core arc at 60-240d band pace). The rest persist as canvas —
         the band sheds slower than the core, by design. */
      if(lcHash(me.code + "#" + me.rank, "cvf") < FILL_TENT_HARDEN_FRAC){ me.tentHarden = 1; thN++; }
    } else if(lcHash(me.code + "#" + me.rank, "cvf") < FILL_CVF_FRAC){ me.canvasFirst = 1; cvfN++; }
  }

  /* ===================================================================
     s106e (ICS-25) — THE EXPANSION POST-PASS (mechanic 2's interaction).
     A seeded fraction of eligible lots (FRAME front + at least one rear
     occupant) EXPAND: the front's footprint grows rearward over its first
     tandem slot via the existing s108 expand machinery; the consumed rear
     occupant is torn down FIRST (teardown completes 0.5d before the annex
     breaks ground — the ruins-gate discipline, spec §2b). The claim is
     proven LEGAL at master-build time: the expanded quad crosses no ROW,
     stands on dry non-camp ground, overlaps no reserved landmark ground and
     no OTHER footprint (SAT vs every master entry except the front and its
     consumed occupants). Scheduling: seeded within the expansion window,
     never before the front is ready (a canvas-first front must have
     HARDENED first) and never before every consumed tent has stood
     FILL_REAR_MIN_STAND_D. A lot whose constraints cannot fit the window
     simply does not expand (opportunistic, never forced). QA:
     ignoreSlotGates skips the legality proof AND the consumption teardown
     (the annex rises over a STANDING tent) — the lotSlots expansion clause
     goes RED (fail-before). ============================================ */
  var expWinA = eventDateToSimDay(FILL_EXP_START_ISO), expWinB = eventDateToSimDay(FILL_EXP_END_ISO);
  var expN = 0, lotsWithRear = 0;
  for(var lx = 0; lx < lots.length; lx++){
    var L = lots[lx];
    if(!L.rearIdxs.length) continue;
    lotsWithRear++;
    var fE = master[L.frontIdx];
    /* annex expansion is a FRAME-BUILDING behaviour (building-types §5). The
       front qualifies when it STANDS AS FRAME by the expansion window: born
       frame-cat, OR a shanty front the s108 upgrade wave rebuilds as frame
       (fillUpgradeArc — the shared law; in this world every frame-cat wave
       placement sits on core blocks, so the residential band's expanding
       fronts are precisely the upgraded ones — the boom's own densification
       path: shanty -> frame -> annex). frontReadyDay = the day its frame
       form is complete + a settling margin. */
    var frontReady;
    if(fE.cat === "frame"){
      frontReady = fE.canvasFirst ? fillCvfArc(fE).rbOn + 15 : fE.appearDay + 30;
    } else if(fE.tentHarden){
      frontReady = fillCvfArc(fE).rbOn + 15;              // a hardened tent stands as frame from its rebuild
    } else {
      var upgF = fillUpgradeArc(fE);
      if(!upgF) continue;                                 // this front never becomes frame — no annex
      frontReady = upgF.rbOn + 15;
    }
    var lotKey = "LOT#" + fE.rank;
    if(lcHash(lotKey, "exp") >= FILL_EXP_FRAC) continue;
    var rear1 = master[L.rearIdxs[0]];
    var dExp = fE.d + FILL_REAR_GAP_M + rear1.d + 0.3;    // the annex reaches over the first tandem slot (+0.3 m working slack, clear of any deeper slot by a full FILL_REAR_GAP_M)
    var ecx = fE.cx - fE.nx * (dExp - fE.d) / 2;          // street face FIXED; the body grows into the block
    var ecz = fE.cz - fE.nz * (dExp - fE.d) / 2;
    var expQuad = fillPoseQuad(ecx, ecz, fE.w, dExp, fE.yaw);
    /* consumed = the first tandem slot ALWAYS + any other lot occupant the
       expanded quad actually reaches (a deeper slot survives by geometry). */
    var consumed = [L.rearIdxs[0]];
    for(var ri = 1; ri < L.rearIdxs.length; ri++){
      var rE = master[L.rearIdxs[ri]];
      if(fillQuadPen(expQuad, fillPoseQuad(rE.cx, rE.cz, rE.w, rE.d, rE.yaw)) > 0) consumed.push(L.rearIdxs[ri]);
    }
    if(!FILL_QA.ignoreSlotGates){
      var expBad = false;
      if(fillQuadCrossesRow(expQuad, ecx, ecz)) expBad = true;
      else if(typeof encampmentZoneAt === "function" && encampmentZoneAt(ecx, ecz, waveCap)) expBad = true;
      else if(typeof isDryLand === "function" && !isDryLand(ecx, ecz, waveStart)) expBad = true;
      else {
        var eRB = Math.hypot(fE.w, dExp) / 2;
        for(var epi = 0; epi < PLACEMENT_INDEX.length; epi++){
          var eq = PLACEMENT_INDEX[epi], edx = ecx - eq.x, edz = ecz - eq.z, emd = eRB + eq.r;
          if(edx*edx + edz*edz < emd*emd){ expBad = true; break; }
        }
        if(!expBad){
          for(var emi = 0; emi < master.length; emi++){
            if(emi === L.frontIdx || consumed.indexOf(emi) >= 0) continue;
            var oE = master[emi];
            if(Math.hypot(oE.cx - ecx, oE.cz - ecz) > eRB + Math.hypot(oE.w, oE.d) / 2 + 0.5) continue;
            if(fillQuadPen(expQuad, fillPoseQuad(oE.cx, oE.cz, oE.w, oE.d, oE.yaw)) > 0){ expBad = true; break; }
          }
        }
      }
      if(expBad) continue;                                // the claim is not legal — no expansion here
    }
    /* scheduling — the front must be READY (frame-formed, computed above)
       and every consumed tent must have STOOD; seed the start inside what
       remains of the window, FRONT-LOADED (hash² — fill:true reasoned: the
       consolidation pressure peaked with the boom itself, so most annexes
       break ground early in the window and the mechanic is visible at the
       Dec-20 noon). */
    var standBy = 0;
    for(var ci2 = 0; ci2 < consumed.length; ci2++)
      standBy = Math.max(standBy, master[consumed[ci2]].appearDay + FILL_REAR_MIN_STAND_D);
    var startMin = Math.max(expWinA, frontReady, standBy + LC_TEARDOWN_DAYS + 1);
    if(startMin >= expWinB - 1) continue;                 // no room left in the window — skip
    var expSeed = lcHash(lotKey, "expday");
    var expStart = startMin + expSeed * expSeed * (expWinB - startMin);
    fE.exp = { start: expStart, on: expStart + FILL_EXP_DUR_D, dExp: dExp, consumes: consumed.map(function(cix){ return master[cix].rank; }) };
    for(var ci3 = 0; ci3 < consumed.length; ci3++){
      if(!FILL_QA.ignoreSlotGates)                        // QA fail-before: the annex rises over a STANDING tent
        master[consumed[ci3]].consumedAt = expStart - LC_TEARDOWN_DAYS - 0.5;
      master[consumed[ci3]].consumedBy = fE.rank;
    }
    expN++;
  }

  /* ===================================================================
     ics26 — PROPERTY-LINE YARD FENCES (backyard texture; activates the
     placement.fences stub). A seeded fraction of RESIDENTIAL-band wave lots
     ring their yard with placeholder post-and-rail runs: two side rails from
     the front building's face reaching past the lot's rear court + a rear
     cross rail. Every P6 clause holds BY CONSTRUCTION OR CHECK: rails are
     parallel to the lot's own axes (the ± lateral/inward frames), posts are
     grounded on terrainHeight (the audit's own measure), and each candidate
     rail is DROPPED if any sample point sits on a right-of-way, inside any
     other footprint, on reserved landmark ground, or crossing another lot's
     accepted rail. Pure lcHash seeding — zero stream draws; deterministic. */
  FENCE_SEGS = []; FENCE_POSTS = []; FILL_FENCE_RENDER = [];
  (function buildYardFences(){
    function segsCross26(ax,az,bx,bz,cx,cz,dx,dz){
      function ccw(px,pz,qx,qz,rx,rz){ return (rz-pz)*(qx-px) > (qz-pz)*(rx-px); }
      return (ccw(ax,az,cx,cz,dx,dz)!==ccw(bx,bz,cx,cz,dx,dz)) && (ccw(ax,az,bx,bz,cx,cz)!==ccw(ax,az,bx,bz,dx,dz));
    }
    for(var lx2 = 0; lx2 < lots.length; lx2++){
      var L2 = lots[lx2], fe2 = master[L2.frontIdx];
      if(fillCoreBlockAt(fe2.cx, fe2.cz, waveCap)) continue;          // residential band only
      if(lcHash("FEN|" + fe2.rank, "on") >= FILL_FENCE_FRAC) continue;
      var own = [L2.frontIdx].concat(L2.rearIdxs);
      var ix2 = -fe2.nx, iz2 = -fe2.nz, ltx = fe2.nz, ltz = -fe2.nx;  // inward + lateral lot axes
      var maxW2 = fe2.w;
      for(var rj2 = 0; rj2 < L2.rearIdxs.length; rj2++) maxW2 = Math.max(maxW2, master[L2.rearIdxs[rj2]].w);
      if(fe2.exp) maxW2 = Math.max(maxW2, fe2.w);
      var halfW2 = maxW2 / 2 + FILL_FENCE_MARGIN_M;
      var backD2 = Math.max(L2.depthUsed, fe2.exp ? (fe2.exp.dExp - fe2.d / 2) : 0, fe2.d / 2) + 1.8;
      var frontS2 = -(fe2.d / 2);
      function FP(s, t){ return { x: fe2.cx + ix2 * s + ltx * t, z: fe2.cz + iz2 * s + ltz * t }; }
      var cand = [
        { a: FP(frontS2, -halfW2), b: FP(backD2, -halfW2) },          // left rail
        { a: FP(frontS2,  halfW2), b: FP(backD2,  halfW2) },          // right rail
        { a: FP(backD2,  -halfW2), b: FP(backD2,  halfW2) }           // rear cross rail
      ];
      var lag2 = FILL_FENCE_LAG_MIN_D + lcHash("FEN|" + fe2.rank, "lag") * (FILL_FENCE_LAG_MAX_D - FILL_FENCE_LAG_MIN_D);
      var bundle = { appearDay: fe2.appearDay + lag2, segs: [], posts: [] };
      for(var ci4 = 0; ci4 < cand.length; ci4++){
        var sg = cand[ci4], segLen = Math.hypot(sg.b.x - sg.a.x, sg.b.z - sg.a.z);
        if(segLen < 1.2) continue;
        var nSmp = Math.max(2, Math.ceil(segLen / 1.5) + 1), okSeg = true;
        for(var si4 = 0; si4 < nSmp && okSeg; si4++){
          var f4 = si4 / (nSmp - 1);
          var px4 = sg.a.x + (sg.b.x - sg.a.x) * f4, pz4 = sg.a.z + (sg.b.z - sg.a.z) * f4;
          if(nearAnyRoad(px4, pz4, 0.25)){ okSeg = false; break; }                        // P6: never on a right-of-way
          for(var pi4 = 0; pi4 < PLACEMENT_INDEX.length && okSeg; pi4++){                 // reserved landmark ground
            var q4 = PLACEMENT_INDEX[pi4], dx4 = px4 - q4.x, dz4 = pz4 - q4.z;
            if(dx4*dx4 + dz4*dz4 < (q4.r + 0.3) * (q4.r + 0.3)) okSeg = false;
          }
          for(var mi4 = 0; mi4 < master.length && okSeg; mi4++){                          // no rail through ANY other footprint
            if(own.indexOf(mi4) >= 0) continue;
            var oE4 = master[mi4];
            if(Math.abs(oE4.cx - px4) > 20 || Math.abs(oE4.cz - pz4) > 20) continue;
            if(fillPointInQuad(fillPoseQuad(oE4.cx, oE4.cz, oE4.w + 0.4, oE4.d + 0.4, oE4.yaw), px4, pz4)) okSeg = false;
          }
        }
        if(!okSeg) continue;
        for(var fs4 = 0; fs4 < FENCE_SEGS.length && okSeg; fs4++){                        // P6: never cross another lot's rail
          var F4 = FENCE_SEGS[fs4];
          if(Math.abs((F4.x0 + F4.x1) / 2 - (sg.a.x + sg.b.x) / 2) > 40) continue;
          if(segsCross26(sg.a.x, sg.a.z, sg.b.x, sg.b.z, F4.x0, F4.z0, F4.x1, F4.z1)) okSeg = false;
        }
        if(!okSeg) continue;
        var rec4 = { x0: sg.a.x, z0: sg.a.z, x1: sg.b.x, z1: sg.b.z, yaw: fe2.yaw, spot: 100000 + fe2.rank, posts: [] };
        FENCE_SEGS.push(rec4); bundle.segs.push(rec4);
        var nPost = Math.max(2, Math.round(segLen / FILL_FENCE_POST_M) + 1);
        for(var pp4 = 0; pp4 < nPost; pp4++){
          var pf4 = pp4 / (nPost - 1);
          var ppx = sg.a.x + (sg.b.x - sg.a.x) * pf4, ppz = sg.a.z + (sg.b.z - sg.a.z) * pf4;
          var post4 = { x: ppx, y: terrainHeight(ppx, ppz), z: ppz };
          FENCE_POSTS.push(post4); rec4.posts.push(post4); bundle.posts.push(post4);
        }
      }
      if(bundle.segs.length) FILL_FENCE_RENDER.push(bundle);
    }
  })();
  ...
  master._wave = { v1: v1Len, placed: waveN, need: need2, startDay: waveStart, capDay: waveCap,
                   ...
                   /* s106d — the boom-window core-canvas exception ledger */
                   coreWave: coreWaveN, coreCanvas: coreCanvasN,
                   coreCanvasCap: Math.ceil(CAD_CORE_CANVAS_FRAC * coreWaveN),
                   coreCanvasFrac: coreWaveN ? +(coreCanvasN / coreWaveN).toFixed(3) : 0,
                   qaUncapCoreCanvas: FILL_QA.uncapCoreCanvas,
                   /* s106e — the slots/arcs ledger (canvas-first + rear-court + expansion) */
                   lots: lots.length, lotsWithRear: lotsWithRear, rearSlots: rearN,
                   rearCap: Math.ceil(FILL_REAR_FRAC * counts2.shanty),
                   canvasFirst: cvfN, tentHarden: thN, expansions: expN,
                   qaIgnoreSlotGates: FILL_QA.ignoreSlotGates,
                   /* ics26 — the hotspot-field coverage ledger (spawner-side; the audit re-measures) */
                   hotspots: hots26.map(function(h){ return { id: h.id, x: +h.x.toFixed(1), z: +h.z.toFixed(1), coreR: h.coreR, midR: h.midR }; }),
                   coverage: cov26 ? cov26.summary() : null,
                   fencedLots: FILL_FENCE_RENDER.length, fenceSegs: FENCE_SEGS.length, fencePosts: FENCE_POSTS.length,
                   qaIgnoreCoverageTargets: FILL_QA.ignoreCoverageTargets };
...
function fillLifecycleOf(e){
  ...
  /* s106e — a CANVAS-FIRST entry is born as canvas (overnight lead, Taylor's
     "as if by magic"), not as its final form; the hardening arc below
     replaces it with the catalog form it was always destined for. */
  var lead = e.canvasFirst ? 2 : fillLeadDays(e);
  evs.push({ type: "construct", start: e.appearDay - lead, on: e.appearDay,
             form: e.canvasFirst ? { cls: "canvas", stories: 1 }
                                 : { cls: e.cat, stories: (e.storyRange && e.storyRange[1]) || 1 } });
  var fire = lcFireHit(e.cx, e.cz);
  /* s106d (ICS-24) — THE CORE-CANVAS SHORT ARC. A boom-window core tent is
     the FIRST torn down and replaced by FRAME on the same lot (the existing
     s108 teardown-replacement machinery — the record's precedent is the
     El Dorado's authored canvas phase on the plaza, shed for its frame
     expansion). Conversion is seeded per tent 2-5 months after ITS spawn
     (FILL_CORE_CONV_MIN/MAX_D), so the arcs spread across late-1849 ->
     mid-1850: scrubbing shows core tents appearing mid-1849, some already
     converting by Dec, and the core trending to zero canvas by the window
     close. A core tent standing in the Dec-24 fire perimeter whose
     conversion would not COMPLETE before the fire skips the arc — the fire
     IS its teardown (burn -> ruins -> clear -> the seeded frame rebuild);
     one that converted early burns as the frame it became. Core tents get
     THIS arc, never the generic ~30% shanty upgrade wave below. */
  if(e.coreCanvas){
    var cv0 = e.appearDay + FILL_CORE_CONV_MIN_D + lcHash(id, "corecnv") * (FILL_CORE_CONV_MAX_D - FILL_CORE_CONV_MIN_D);
    var cvOn = cv0 + LC_TEARDOWN_DAYS;
    var cvLead = 14 + Math.floor(lcHash(id, "corerb") * 10);
    if(!fire || cvOn + 2 + cvLead < fire.day - 1){
      var cvrS = cvOn + 2;
      if(LC_QA.breakRuinsGate) cvrS = cv0 + 0.5;          // QA: rebuild before the lot is cleared (ruins-gate fail-before)
      evs.push({ type: "teardown", start: cv0, on: cvOn });
      evs.push({ type: "rebuild", start: cvrS, on: cvrS + cvLead,
                 form: { cls: "frame", stories: (e.storyRange && e.storyRange[1]) || 1 } });
    }
  } else if(e.rearOf != null){
    /* s106e — THE REAR-SLOT LAW (mechanic 2). A rear-court tent runs its own
       arc: it appears with its lot's demand rank and STAYS CANVAS through the
       window (rear slots lag — the operator's texture; it never takes the
       generic ~30% shanty upgrade below and never a canvas-first arc: it IS
       canvas). Its only in-window exit is CONSUMPTION — the front building's
       expansion claims its ground: a teardown that COMPLETES 0.5d before the
       annex breaks ground (scheduled by the expansion post-pass; the ruins-
       gate discipline holds — teardown clears as it goes, spec §2b), then
       cleared -> gone: the annex takes the ground, the tent is not rebuilt.
       A consumed tent standing in a fire perimeter whose teardown would not
       complete first skips the arc — the fire is its end (unreachable today:
       rear tents are never core ground and the Dec-24 perimeter is; kept as
       the same defensive guard the core arc carries). */
    if(e.consumedAt != null && (!fire || e.consumedAt + LC_TEARDOWN_DAYS < fire.day - 1)){
      evs.push({ type: "teardown", start: e.consumedAt, on: e.consumedAt + LC_TEARDOWN_DAYS });
    }
  } else if(e.canvasFirst){
    /* s106e — THE CANVAS-FIRST HARDENING ARC (mechanic 1). The residential-
       band generalization of the s106d core arc: the canvas the entry was
       born as is torn down and its FINAL catalog form rises on the same lot
       (the s108 teardown-for-replacement machinery), seeded FILL_RES_CONV_
       MIN..MAX_D days after ITS OWN spawn — months, slower than the core's
       60-150d (the core sheds canvas fastest; the band hardens through
       winter into spring 1850). Zone-modulated by construction: core entries
       take the s106d branch above, encampment-zone tents are the tent
       master's (never this master's), rear slots stay canvas (above). */
    var arcCvf = fillCvfArc(e);
    if(!fire || arcCvf.rbOn < fire.day - 1){
      var cvfRbS = arcCvf.rbStart;
      if(LC_QA.breakRuinsGate) cvfRbS = arcCvf.tdStart + 0.5;   // QA: rebuild before the lot is cleared (ruins-gate fail-before)
      evs.push({ type: "teardown", start: arcCvf.tdStart, on: arcCvf.tdOn });
      evs.push({ type: "rebuild", start: cvfRbS, on: cvfRbS + arcCvf.lead,
                 form: { cls: e.cat, stories: (e.storyRange && e.storyRange[1]) || 1 } });
    }
  } else if(e.tentHarden){
    /* s106e — THE BAND TENT->FRAME HARDENING ARC (mechanic 1, the tent side).
       The same fillCvfArc numbers (60-240d after ITS OWN spawn), the same
       teardown-for-replacement machinery, rebuilt as FRAME (the documented
       tent->frame replacement cycle, fill-density-model §4). The core's
       tents keep the s106d 60-150d law (branch above); the band hardens
       slower, and only a seeded fraction at all. */
    var arcTh = fillCvfArc(e);
    if(!fire || arcTh.rbOn < fire.day - 1){
      var thRbS = arcTh.rbStart;
      if(LC_QA.breakRuinsGate) thRbS = arcTh.tdStart + 0.5;     // QA: rebuild before the lot is cleared (ruins-gate fail-before)
      evs.push({ type: "teardown", start: arcTh.tdStart, on: arcTh.tdOn });
      evs.push({ type: "rebuild", start: thRbS, on: thRbS + arcTh.lead,
                 form: { cls: "frame", stories: (e.storyRange && e.storyRange[1]) || 1 } });
    }
  } else if(!fire){
    /* the s108 ~30% shanty upgrade wave — the SAME law as ever, read through
       the shared fillUpgradeArc helper (s106e factored it out so the
       expansion post-pass and this compiler never disagree about who becomes
       frame — measurement law). Event-identical to the inline original. */
    var upg = fillUpgradeArc(e);
    if(upg){
      evs.push({ type: "teardown", start: upg.t0, on: upg.tOn });
      var rStart = upg.rbStart;
      if(LC_QA.breakRuinsGate) rStart = upg.t0 + 0.5;           // QA: rebuild before the lot is cleared
      evs.push({ type: "rebuild", start: rStart, on: rStart + 21,
                 form: { cls: "frame", stories: (e.storyRange && e.storyRange[1]) || 1 } });
    }
  }
  /* s106e — THE FRONT EXPANSION (mechanic 2's interaction). The annex the
     post-pass granted: an s108 expand event over the consumed rear slot's
     cleared ground (the consumed occupant's teardown completed first — its
     own arc above). The renderer + audits read the expanded footprint from
     the derive (deriveFillSet applies e.exp from expand start). */
  if(e.exp && !fire){
    evs.push({ type: "expand", start: e.exp.start, on: e.exp.on,
               form: { cls: "frame", stories: (e.storyRange && e.storyRange[1]) || 1 } });   // an expanding front stands as FRAME by grant (post-pass eligibility)
  }
  if(fire){
    var b = fire.day;
...
function deriveFillSet(day){
  ...
  for(var i = 0; i < master.length; i++){
    ...
    var form = st.form || {};
    /* s106e — the EXPANDED footprint: from the annex's break-ground day the
       front occupies its grown claim (street face fixed, body deeper into
       the block — over the consumed rear slot's cleared ground). The derive
       is the ONE pose source for the renderer AND the drawn-OBB audits, so
       the measured box is the drawn box (measurement law). */
    var dd = e.d, dcx = e.cx, dcz = e.cz, expanded = false;
    if(e.exp && day >= e.exp.start - 1e-9){
      dd = e.exp.dExp;
      dcx = e.cx - e.nx * (e.exp.dExp - e.d) / 2;
      dcz = e.cz - e.nz * (e.exp.dExp - e.d) / 2;
      expanded = true;
    }
    out.push({ code: e.code, cat: e.cat, cls: form.cls || e.cat, material: e.material, subtype: e.subtype,
               cx: dcx, cz: dcz, w: e.w, d: dd, yaw: e.yaw, nx: e.nx, nz: e.nz,
               ...
               coreCanvas: e.coreCanvas || 0,             // s106d: boom-window core-canvas exception entry
               canvasFirst: e.canvasFirst || 0,           // s106e: canvas-first hardening arc entry
               tentHarden: e.tentHarden || 0,             // s106e: band tent->frame hardening arc entry
               rearOf: e.rearOf != null ? e.rearOf : null, slot: e.slot || 0,   // s106e: rear-court slot occupant
               expanded: expanded, consumedBy: e.consumedBy != null ? e.consumedBy : null,
...
function rebuildFill(){
  ...
  for(var i = 0; i < set.length; i++){
    ...
    /* s106d: a tent whose CURRENT lifecycle form has become frame (the
       core-canvas conversion rebuild, or any tent->frame arc) draws as the
       box massing of what now stands, not as canvas — the tent vocabulary
       covers only the canvas phases (construct/stand/teardown/cleared). */
    /* s106e: a CANVAS-FIRST entry draws with the tent vocabulary while its
       current lifecycle form is still canvas (construct/stand/teardown of
       the canvas phase) — the same routing rule the core-canvas conversion
       established: the tent vocabulary covers exactly the canvas phases;
       once the hardening rebuild's form takes over, the box massing does. */
    if(((set[i].subtype === "tent" && set[i].cls !== "frame") || set[i].cls === "canvas") &&
       typeof TENT_VARIANTS !== "undefined"){ tentPartsInto(tentParts, set[i]); continue; }
  ...
  /* ics26 — placeholder yard fences (post-and-rail): date-gated by each lot's
     seeded fence day; posts grounded on terrainHeight (the P6 measure), rails
     spanned post-to-post so runs follow the grade. One merged mesh. */
  var fenceParts = [], FEN_COL = new THREE.Color(0x8f8271);
  for(var fb = 0; fb < FILL_FENCE_RENDER.length; fb++){
    var bun = FILL_FENCE_RENDER[fb];
    if(bun.appearDay > simDay) continue;
    for(var fsg = 0; fsg < bun.segs.length; fsg++){
      var seg = bun.segs[fsg], P5 = seg.posts;
      for(var fp5 = 0; fp5 < P5.length; fp5++){
        var po = P5[fp5];
        var postGeo = makeBoxLocal(0.09, 0.75, 0.09, FEN_COL);
        bake(postGeo, new THREE.Vector3(po.x, po.y, po.z), seg.yaw);
        fenceParts.push(postGeo);
        if(fp5 > 0){
          var pa = P5[fp5 - 1], span = Math.hypot(po.x - pa.x, po.z - pa.z);
          if(span > 0.01){
            var railGeo = makeBoxLocal(0.06, 0.05, span, FEN_COL);
            var railYaw = Math.atan2(po.x - pa.x, po.z - pa.z);
            bake(railGeo, new THREE.Vector3((pa.x + po.x) / 2, (pa.y + po.y) / 2 + 0.5, (pa.z + po.z) / 2), railYaw);
            fenceParts.push(railGeo);
          }
        }
      }
    }
  ...
  if(fenceParts.length){
    var fm = new THREE.Mesh(mergeGeoms(fenceParts), LM_MAT);
    fm.frustumCulled = false;
    fm.userData.fillFences = true;
    FILL_GROUP.add(fm);
  }
...
(function registerFillAudits(){
  ...
  registerAudit("buildings", "fillFootprintsOBB", function(){
    ...
    days.forEach(function(day){
      /* s106e scope amendment: the DETACHED law governs what physically
         STANDS — bodies and their rubble (constructing/complete/expanding/
         burning/teardown/ruins/clearing). A lot at cleared/gone/absent has
         NOTHING standing to be detached from, and the s106e expansion
         lawfully claims a consumed rear slot's cleared ground — measuring a
         gone tent's empty pad against the annex that replaced it would gate
         on a structure that no longer exists. The expansion's own packing
         legality (vs everything NOT consumed) is gated by buildings.lotSlots. */
      var set = deriveFillSet(day).filter(function(e){
        return e.phase !== "cleared" && e.phase !== "gone" && e.phase !== "absent";
      });
      var quads = set.map(fillQuad);
  ...
  /* ---- zoneCoreCanvas (s106d, ICS-24) — THE BOOM-WINDOW CORE-CANVAS LAW.
     The operator's dated, bounded exception made standing (rules-first):
       (1) THE CAP — canvas standing on core-block ground among the WAVE
           placements never exceeds ceil(CAD_CORE_CANVAS_FRAC x core-block
           wave placements), and the exception is REAL (>= 1 core tent
           placed). Measured on the master with fillCoreBlockAt — the SAME
           ground measure the spawner gated on (measurement law).
       (2) THE WINDOW — every core-canvas entry is born INSIDE
           [CAD_CORE_CANVAS_START_ISO, CAD_CORE_CANVAS_END_ISO); ZERO core
           canvas stands at any pre-window date (the 1847/48 census gates +
           the pre-boom spring); zero NEW canvas is admitted outside it.
       (3) THE GATE IS LAWFUL — cadZoneGate probes at a real core-canvas
           point: permit+in-window ADMITS; permit outside the window (before
           AND after) REJECTS (the permit is void, not a bypass); no permit
           in-window REJECTS (the standing law — the zoneLaw audit's general
           probe unchanged). The encampment record-wins path is DISTINCT:
           this law governs only the in-town wave on core blocks.
       (4) THE SHORT ARC — every core tent either converts (s108 teardown ->
           frame rebuild seeded FILL_CORE_CONV_MIN..MAX_D days after ITS
           spawn — the core sheds canvas across late-1849 -> mid-1850, some
           conversions already visible by Dec) or stands in the authored
           Dec-24 fire perimeter (the fire is its teardown); ZERO core canvas
           still stands by 1850-06-01.
     Fail-before: __P1850_FILL_QA({uncapCoreCanvas:true}) drops the fraction
     cap — canvas floods the core slots and (1) goes RED; knob off -> GREEN
     (proven in probe-s106d). ---- */
  registerAudit("buildings", "zoneCoreCanvas", function(){
    var master = buildFillMaster(), wv = master._wave || { v1: master.length };
    var capDay = wv.capDay != null ? wv.capDay : eventDateToSimDay(FILL_1849_CAP_ISO);
    var winA = eventDateToSimDay(CAD_CORE_CANVAS_START_ISO), winB = eventDateToSimDay(CAD_CORE_CANVAS_END_ISO);
    var problems = [];
    // (1) the cap, re-measured on the master (never trusting the spawner's ledger)
    var coreWave = 0, coreCanvas = 0, canvasEntries = [];
    for(var i = (wv.v1 || 0); i < master.length; i++){
      var e = master[i];
      if(!fillCoreBlockAt(e.cx, e.cz, capDay)) continue;
      coreWave++;
      if(e.subtype === "tent"){ coreCanvas++; canvasEntries.push(e); }
    }
    var cap = Math.ceil(CAD_CORE_CANVAS_FRAC * coreWave);
    if(coreCanvas > cap) problems.push("core canvas " + coreCanvas + " over the bounded cap ceil(" + CAD_CORE_CANVAS_FRAC + " x " + coreWave + ") = " + cap);
    if(coreCanvas < 1) problems.push("ZERO core canvas placed — the dated exception is vacuous");
    // (2) the window: births inside it; zero canvas standing pre-window
    canvasEntries.forEach(function(ce){
      if(ce.appearDay < winA - 1e-6 || ce.appearDay >= winB)
        problems.push("core tent " + ce.code + "#" + ce.rank + " born outside the boom window (day " + ce.appearDay.toFixed(1) + ")");
    });
    /* NOTE the scope: this law governs the WAVE's canvas on core blocks. The
       v1 village legally holds a couple of 1847-era tents whose ground the
       era-GROWN core swallowed later (they were residential-band ground on
       their build day — era growth is not a retroactive eviction), and the
       encampment-zone tents are governed by THEIR record-wins law. Neither
       is this exception's subject. */
    var preWindow = [];
    ["1847-04-01", "1848-04-01", "1849-06-01"].forEach(function(iso){
      var day = eventDateToSimDay(iso);
      var n = deriveFillSet(day).filter(function(f){ return f.wave === 1849 && f.subtype === "tent" && fillCoreBlockAt(f.cx, f.cz, capDay); }).length;
      preWindow.push({ date: iso, waveCoreCanvasStanding: n });
      if(n > 0) problems.push(iso + ": " + n + " wave core canvas standing before the window opens");
    });
    // (3) the gate probes at a REAL core-canvas point (guaranteed core ground
    // outside any documented camp; the point must also read commercial-core at
    // the out-of-window dates so the probe isolates the WINDOW, not zone drift)
    var dayIn = eventDateToSimDay("1849-09-15"), dayPre = eventDateToSimDay("1849-05-01"), dayPost = eventDateToSimDay("1850-04-01");
    var pt = null;
    for(var ci = 0; ci < canvasEntries.length; ci++){
      var c = canvasEntries[ci];
      if(fillCoreBlockAt(c.cx, c.cz, dayPre) && fillCoreBlockAt(c.cx, c.cz, dayPost)){ pt = { x: c.cx, z: c.cz }; break; }
    }
    var probes = [], probeFail = 0;
    function gateProbe(name, day, opts, wantOk){
      if(!pt){ probes.push({ probe: name, located: false }); probeFail++; return; }
      var g = cadZoneGate("tent", pt.x, pt.z, day, opts);
      var ok = (g.ok === wantOk);
      if(!ok) probeFail++;
      probes.push({ probe: name, located: true, gotOk: g.ok, reason: g.reason, pass: ok });
    }
    gateProbe("permit-in-window-admits",    dayIn,   { coreCanvasPermit: true }, true);
    gateProbe("permit-before-window-rejects", dayPre, { coreCanvasPermit: true }, false);
    gateProbe("permit-after-window-rejects",  dayPost, { coreCanvasPermit: true }, false);
    gateProbe("no-permit-in-window-rejects",  dayIn,   null,                      false);
    if(probeFail) problems.push(probeFail + " gate probes failed (the window law is not real at the gate)");
    // (4) the short arcs + the core trending to zero
    var convDays = [], convByDec20 = 0, fireConsumed = 0, dec20 = eventDateToSimDay("1849-12-20");
    canvasEntries.forEach(function(ce){
      var id = ce.code + "#" + ce.rank;
      var fire = lcFireHit(ce.cx, ce.cz);
      var evs = fillLifecycleOf(ce).events, td = null, rb = null;
      for(var k = 0; k < evs.length; k++){
        if(evs[k].type === "teardown" && !td) td = evs[k];
        if(evs[k].type === "rebuild" && td && !rb) rb = evs[k];
      }
      if(!td){
        if(fire){ fireConsumed++; return; }               // the Dec-24 fire is its teardown
        problems.push("core tent " + id + " has NO conversion arc and is not in the fire perimeter");
        return;
      }
      var dly = td.start - ce.appearDay;
      convDays.push(+dly.toFixed(1));
      if(dly < FILL_CORE_CONV_MIN_D - 0.5 || dly > FILL_CORE_CONV_MAX_D + 0.5)
        problems.push("core tent " + id + " conversion delay " + dly.toFixed(1) + "d outside [" + FILL_CORE_CONV_MIN_D + "," + FILL_CORE_CONV_MAX_D + "]");
      if(!rb || !rb.form || rb.form.cls !== "frame")
        problems.push("core tent " + id + " teardown not followed by a FRAME rebuild (teardown-for-replacement law)");
      if(td.start <= dec20) convByDec20++;
    });
    if(canvasEntries.length > 0 && convByDec20 + fireConsumed < 1)
      problems.push("no core tent converts (or burns) by Dec-20 — the in-window shedding is invisible");
    var standingMid1850 = deriveFillSet(eventDateToSimDay("1850-06-01")).filter(function(f){
      return f.wave === 1849 && f.subtype === "tent" && f.cls !== "frame" && fillCoreBlockAt(f.cx, f.cz, capDay) &&
             (f.phase === "complete" || f.phase === "constructing" || f.phase === "expanding");
    }).length;
    if(standingMid1850 > 0) problems.push(standingMid1850 + " wave core canvas still standing at 1850-06-01 (the core never shed it)");
    return { pass: problems.length === 0, law: "zoneCoreCanvas (boom-window core-canvas exception, s106d)",
             windowISO: [CAD_CORE_CANVAS_START_ISO, CAD_CORE_CANVAS_END_ISO], frac: CAD_CORE_CANVAS_FRAC,
             coreWave: coreWave, coreCanvas: coreCanvas, cap: cap,
             measuredFrac: coreWave ? +(coreCanvas / coreWave).toFixed(3) : 0,
             preWindow: preWindow, gate: { failures: probeFail, probes: probes },
             arcs: { conversions: convDays.length, fireConsumed: fireConsumed, convertedByDec20: convByDec20,
                     delaysD: { min: convDays.length ? Math.min.apply(null, convDays) : null,
                                max: convDays.length ? Math.max.apply(null, convDays) : null },
                     standingAt1850_06: standingMid1850 },
             problems: problems.slice(0, 10),
             qaKnobs: { uncapCoreCanvas: FILL_QA.uncapCoreCanvas } };
  });

  /* ---- lotSlots (s106e, ICS-25) — CANVAS-FIRST LOT ARCS + SLOTS: the
     operator's two mechanics made standing rules (rules-first):
       (1) PACKING LEGALITY — every rear-slot occupant is a TENT posed in its
           lot's rear court (behind its front, inside the front's width band,
           within the tandem depth), on dry non-core non-camp ground, crossing
           no ROW, on no reserved landmark ground, detached from every other
           footprint. Measured on the MASTER with the same geometry measures
           the spawner gated on (fillCoreBlockAt / segCrossesQuad / SAT —
           measurement law).
       (2) ARC LEGALITY PER ZONE — canvas-first arcs stand ONLY on
           residential-band ground (the core keeps the s106d 60-150d law —
           zoneCoreCanvas gates it; the encampment zones' tents belong to the
           tent master, never this wave): born as canvas, hardening teardown
           seeded FILL_RES_CONV_MIN..MAX_D days after ITS spawn, rebuilt as
           the entry's own final catalog form. A rear tent STAYS canvas — no
           upgrade/hardening arc ever; its only exit is consumption.
       (3) EXPANSION CONSUMES THE SLOT — every expanding front is frame, its
           consumed occupants' teardowns COMPLETE before the annex breaks
           ground (the ruins-gate discipline), each consumed tent STOOD at
           least FILL_REAR_MIN_STAND_D first, a canvas-first front HARDENED
           first, and the expanded claim is itself legal (no ROW / reserved /
           other-footprint overlap). At least one expansion breaks ground by
           the Dec-20 noon (the mechanic is visible in-window, not vacuous).
       (4) THE RAILS — no slot/arc entry pre-dates the wave (the 1847/48
           censuses cannot be touched), and the rear-slot count respects the
           streaming cap ceil(FILL_REAR_FRAC x shanty placements) — rear
           tents are a RESHUFFLE of the wave's shanty share, never net-new.
     Fail-before: __P1850_FILL_QA({ignoreSlotGates:true}) poses rear tents on
     the STREET side of their fronts with every gate skipped AND grants
     expansions without scheduling the consumption teardowns — (1) and (3) go
     RED; knob off -> GREEN (proven in probe-s106e). ---- */
  registerAudit("buildings", "lotSlots", function(){
    var master = buildFillMaster(), wv = master._wave || { v1: master.length };
    var capDay = wv.capDay != null ? wv.capDay : eventDateToSimDay(FILL_1849_CAP_ISO);
    var startDay = wv.startDay != null ? wv.startDay : eventDateToSimDay(FILL_1849_START_ISO);
    var dec20 = eventDateToSimDay("1849-12-20");
    var segs = (typeof PLACEMENT_STREET_SEGS !== "undefined") ? PLACEMENT_STREET_SEGS : [];
    var lmq = landmarkQuadsAt(capDay);
    var problems = [], rears = [], cvfs = [], ths = [], exps = [];
    for(var i = (wv.v1 || 0); i < master.length; i++){
      var e = master[i];
      if(e.rearOf != null) rears.push(e);
      if(e.canvasFirst) cvfs.push(e);
      if(e.tentHarden) ths.push(e);
      if(e.exp) exps.push(e);
    }
    // (0) the mechanics are REAL, not vacuous
    if(!rears.length) problems.push("ZERO rear-slot occupants placed — the slot mechanic is vacuous");
    if(!cvfs.length)  problems.push("ZERO canvas-first arcs seeded — the arc mechanic is vacuous");
    if(!ths.length)   problems.push("ZERO band tent-harden arcs seeded — the tent side of the arc mechanic is vacuous");
    if(!exps.length)  problems.push("ZERO expansions granted — the consumption mechanic is vacuous");
    // (1) PACKING LEGALITY — every rear occupant, on the master's own geometry
    var packViol = 0, packSample = [];
    function packBad(e2, why){ packViol++; if(packSample.length < 8) packSample.push({ rank: e2.rank, code: e2.code, why: why }); }
    rears.forEach(function(re){
      var f = master[re.rearOf];
      if(!f || f.wave !== 1849 || f.rearOf != null){ packBad(re, "front-unresolvable"); return; }
      if(re.subtype !== "tent"){ packBad(re, "rear-occupant-not-canvas"); return; }
      var rvx = re.cx - f.cx, rvz = re.cz - f.cz;
      var projIn = -(rvx * f.nx + rvz * f.nz);                       // + = into the block (behind the front)
      var projT  = rvx * f.nz + rvz * (-f.nx);                       // lateral, along the frontage
      if(projIn <= 0) packBad(re, "not-behind-front (street side)");
      else if(projIn > f.d / 2 + 2 * (FILL_REAR_GAP_M + 4.5)) packBad(re, "beyond-the-lot-rear-court");
      /* width band: the rear centreline stays inside the front's width band —
         a rear tent WIDER than a narrow front is centred (lat 0) and its own
         detached/ROW/overlap clauses govern its sides, so the band term
         floors at zero (Math.max), never a negative impossibility. */
      if(Math.abs(projT) > Math.max(0, (f.w - re.w) / 2) + 0.3) packBad(re, "outside-front-width-band");
      if(re.appearDay < f.appearDay - 1e-6) packBad(re, "rear-develops-before-front");
      if(fillCoreBlockAt(re.cx, re.cz, capDay)) packBad(re, "on-core-ground");
      if(typeof encampmentZoneAt === "function" && encampmentZoneAt(re.cx, re.cz, capDay)) packBad(re, "on-documented-camp-ground");
      if(typeof isDryLand === "function" && !isDryLand(re.cx, re.cz, startDay)) packBad(re, "on-water");
      var q = fillQuad(re);
      for(var sj = 0; sj < segs.length; sj++){
        var mx = (segs[sj].x0 + segs[sj].x1) / 2, mz = (segs[sj].z0 + segs[sj].z1) / 2;
        if(Math.hypot(mx - re.cx, mz - re.cz) > 90) continue;
        if(segCrossesQuad(q, segs[sj])){ packBad(re, "crosses-row:" + segs[sj].id); break; }
      }
      for(var lj = 0; lj < lmq.length; lj++)
        if(penetration(q, lmq[lj]) > 0.05){ packBad(re, "on-reserved-landmark-ground"); break; }
      for(var oj = 0; oj < master.length; oj++){
        if(oj === re.rank) continue;
        var oE = master[oj];
        if(Math.hypot(oE.cx - re.cx, oE.cz - re.cz) > Math.hypot(re.w, re.d) / 2 + Math.hypot(oE.w, oE.d) / 2 + 0.5) continue;
        if(penetration(q, fillQuad(oE)) > 0.05){ packBad(re, "overlaps:" + oE.code + "#" + oE.rank); break; }
      }
    });
    if(packViol) problems.push(packViol + " rear-slot packing violations");
    // (2) ARC LEGALITY PER ZONE
    var arcViol = 0, arcSample = [], delays = [];
    cvfs.forEach(function(ce){
      var id = ce.code + "#" + ce.rank;
      if(fillCoreBlockAt(ce.cx, ce.cz, capDay)){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "canvas-first-on-core (the core keeps the s106d law)" }); }
      if(ce.subtype === "tent"){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "canvas-first-tent (a tent IS canvas)" }); }
      var evs = fillLifecycleOf(ce).events, con = evs[0], td = null, rb = null;
      for(var k = 0; k < evs.length; k++){
        if(evs[k].type === "teardown" && !td) td = evs[k];
        if(evs[k].type === "rebuild" && td && !rb) rb = evs[k];
      }
      if(!con || con.type !== "construct" || !con.form || con.form.cls !== "canvas"){
        arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "not-born-as-canvas" });
      }
      if(!td){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "no-hardening-teardown" }); return; }
      var dly = td.start - ce.appearDay;
      delays.push(+dly.toFixed(1));
      if(dly < FILL_RES_CONV_MIN_D - 0.5 || dly > FILL_RES_CONV_MAX_D + 0.5){
        arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "hardening-delay " + dly.toFixed(1) + "d outside [" + FILL_RES_CONV_MIN_D + "," + FILL_RES_CONV_MAX_D + "]" });
      }
      if(!rb || !rb.form || rb.form.cls !== ce.cat){
        arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "hardening-not-to-final-form" });
      }
    });
    ths.forEach(function(te){
      var id = te.code + "#" + te.rank;
      if(fillCoreBlockAt(te.cx, te.cz, capDay)){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "tent-harden-on-core (the core keeps the s106d law)" }); }
      if(te.subtype !== "tent"){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "tent-harden-on-a-non-tent" }); }
      var evs2 = fillLifecycleOf(te).events, td3 = null, rb3 = null;
      for(var k3 = 0; k3 < evs2.length; k3++){
        if(evs2[k3].type === "teardown" && !td3) td3 = evs2[k3];
        if(evs2[k3].type === "rebuild" && td3 && !rb3) rb3 = evs2[k3];
      }
      if(!td3){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "no-hardening-teardown" }); return; }
      var dly2 = td3.start - te.appearDay;
      delays.push(+dly2.toFixed(1));
      if(dly2 < FILL_RES_CONV_MIN_D - 0.5 || dly2 > FILL_RES_CONV_MAX_D + 0.5){
        arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "hardening-delay " + dly2.toFixed(1) + "d outside [" + FILL_RES_CONV_MIN_D + "," + FILL_RES_CONV_MAX_D + "]" });
      }
      if(!rb3 || !rb3.form || rb3.form.cls !== "frame"){
        arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "tent-hardening-not-to-frame (the §4 replacement law)" });
      }
    });
    rears.forEach(function(re){
      var id = re.code + "#" + re.rank;
      var evs = fillLifecycleOf(re).events, hasRb = false, hasTd = false;
      for(var k = 0; k < evs.length; k++){
        if(evs[k].type === "rebuild") hasRb = true;
        if(evs[k].type === "teardown") hasTd = true;
      }
      if(hasRb){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "rear-tent-rebuilds (rear slots stay canvas)" }); }
      if(hasTd && re.consumedAt == null){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "rear-teardown-without-consumption" }); }
      if(re.canvasFirst || re.tentHarden){ arcViol++; if(arcSample.length < 8) arcSample.push({ id: id, why: "rear-marked-for-a-hardening-arc (rear slots stay canvas)" }); }
    });
    if(arcViol) problems.push(arcViol + " arc-legality violations");
    // (3) EXPANSION CONSUMES THE SLOT (the ruins-gate discipline holds)
    var expViol = 0, expSample = [], expByDec20 = 0;
    function expBad(f2, why){ expViol++; if(expSample.length < 8) expSample.push({ front: f2.code + "#" + f2.rank, why: why }); }
    exps.forEach(function(fe){
      /* the expanding front must STAND AS FRAME when the annex breaks ground
         — measured on its OWN compiled events (the law's terms): some
         construct/rebuild whose form is frame completes on or before
         exp.start (covers frame-cat fronts AND upgraded shanty fronts). */
      var fevs = fillLifecycleOf(fe).events, frameBy = false;
      for(var fk = 0; fk < fevs.length; fk++){
        var fv = fevs[fk];
        if((fv.type === "construct" || fv.type === "rebuild") && fv.form && fv.form.cls === "frame" && fv.on <= fe.exp.start + 1e-6){ frameBy = true; break; }
      }
      if(!frameBy) expBad(fe, "front-not-frame-formed-by-expansion");
      if(!fe.exp.consumes || !fe.exp.consumes.length) expBad(fe, "expansion-consumes-nothing");
      if(fe.exp.start <= dec20) expByDec20++;
      if(fe.canvasFirst && fe.exp.start < fillCvfArc(fe).rbOn - 1e-6) expBad(fe, "expands-before-hardening-completes");
      var dExp = fe.exp.dExp;
      var ecx = fe.cx - fe.nx * (dExp - fe.d) / 2, ecz = fe.cz - fe.nz * (dExp - fe.d) / 2;
      var eq2 = fillQuad({ cx: ecx, cz: ecz, w: fe.w, d: dExp, yaw: fe.yaw });
      (fe.exp.consumes || []).forEach(function(cr){
        var ce2 = master[cr];
        if(!ce2 || ce2.rearOf !== fe.rank){ expBad(fe, "consumed#" + cr + "-not-this-lot's-rear"); return; }
        var evs2 = fillLifecycleOf(ce2).events, td2 = null;
        for(var k2 = 0; k2 < evs2.length; k2++) if(evs2[k2].type === "teardown"){ td2 = evs2[k2]; break; }
        if(!td2){ expBad(fe, "consumed#" + cr + "-never-torn-down (annex over a standing tent)"); return; }
        if(td2.on > fe.exp.start + 1e-6) expBad(fe, "consumed#" + cr + "-teardown-completes-after-annex-breaks-ground");
        if(td2.start < ce2.appearDay + FILL_REAR_MIN_STAND_D - 0.5) expBad(fe, "consumed#" + cr + "-torn-down-before-it-stood " + FILL_REAR_MIN_STAND_D + "d");
      });
      for(var sj2 = 0; sj2 < segs.length; sj2++){
        var mx2 = (segs[sj2].x0 + segs[sj2].x1) / 2, mz2 = (segs[sj2].z0 + segs[sj2].z1) / 2;
        if(Math.hypot(mx2 - ecx, mz2 - ecz) > 90) continue;
        if(segCrossesQuad(eq2, segs[sj2])){ expBad(fe, "annex-crosses-row:" + segs[sj2].id); break; }
      }
      for(var lj2 = 0; lj2 < lmq.length; lj2++)
        if(penetration(eq2, lmq[lj2]) > 0.05){ expBad(fe, "annex-on-reserved-landmark-ground"); break; }
      for(var oj2 = 0; oj2 < master.length; oj2++){
        if(oj2 === fe.rank || (fe.exp.consumes || []).indexOf(oj2) >= 0) continue;
        var oE2 = master[oj2];
        if(Math.hypot(oE2.cx - ecx, oE2.cz - ecz) > Math.hypot(fe.w, dExp) / 2 + Math.hypot(oE2.w, oE2.d) / 2 + 0.5) continue;
        if(penetration(eq2, fillQuad(oE2)) > 0.05){ expBad(fe, "annex-overlaps:" + oE2.code + "#" + oE2.rank); break; }
      }
    });
    if(exps.length && expByDec20 < 1) problems.push("no expansion breaks ground by Dec-20 — the consumption mechanic is invisible in-window");
    if(expViol) problems.push(expViol + " expansion-law violations");
    // (4) THE RAILS — nothing pre-dates the wave; the rear share is capped
    var railViol = 0;
    rears.concat(cvfs).concat(ths).forEach(function(e3){
      if(e3.appearDay < startDay - 0.5) railViol++;
    });
    if(railViol) problems.push(railViol + " slot/arc entries pre-date the wave (census-gate hazard)");
    var shantyWave = 0;
    for(var m2 = (wv.v1 || 0); m2 < master.length; m2++) if(master[m2].cat === "shanty") shantyWave++;
    var rearCapN = Math.ceil(FILL_REAR_FRAC * shantyWave);
    if(rears.length > rearCapN) problems.push("rear slots " + rears.length + " over the cap ceil(" + FILL_REAR_FRAC + " x " + shantyWave + ") = " + rearCapN);
    return { pass: problems.length === 0, law: "lotSlots (canvas-first lot arcs + slots, s106e)",
             counts: { lots: wv.lots, lotsWithRear: wv.lotsWithRear, rearSlots: rears.length, rearCap: rearCapN,
                       canvasFirst: cvfs.length, tentHarden: ths.length,
                       expansions: exps.length, expansionsByDec20: expByDec20,
                       waveShanty: shantyWave, wavePlaced: master.length - (wv.v1 || 0) },
             packing: { violations: packViol, sample: packSample },
             arcs: { violations: arcViol, sample: arcSample,
                     hardenDelaysD: { min: delays.length ? Math.min.apply(null, delays) : null,
                                      max: delays.length ? Math.max.apply(null, delays) : null } },
             expansion: { violations: expViol, sample: expSample },
             problems: problems.slice(0, 10),
             qaKnobs: { ignoreSlotGates: FILL_QA.ignoreSlotGates } };
  });

  /* ---- blockCoverage (ics26, ICS-26) — THE MEASURED PER-BAND COVERAGE LAW
     (RES-4, research/block-coverage-1849-53.md §6). The Dec-20 world is
     measured per BLOCK — summed standing footprint area (derived fill in a
     physical phase + landmarks, the drawn OBBs) over the block's DRY area —
     and gated per MEASUREMENT BAND (the RES-4 definitions: plaza distance +
     the wharf-corridor exception):
       (1) CORE (plaza-facing + ~1-2 blocks): band mean within 35±8%, AND a
           real per-block spread (blocks sample the measured 30-65% range —
           never one painted mean);
       (2) MID (~2-4 blocks): mean within 12±5%;
       (3) FRINGE (beyond): mean within 3±2%;
       (4) WATERFRONT-FILL (blocks on the ACTIVE wharf-foot corridors —
           derived from the pier record, the Sansome-Battery finding): mean
           at CORE grade (35±8%), despite mid-band distance;
       (5) every gated band is REAL (enough eligible blocks to measure).
     Sept-15 is reported informational. Fail-before:
     __P1850_FILL_QA({ignoreCoverageTargets:true}) reverts the wave to the
     flat plaza-radial fill (no field, no budgets, no size grading) — the
     band aggregates drift and this audit goes RED; knob off -> GREEN. ---- */
  registerAudit("buildings", "blockCoverage", function(){
    var m = fillBlockCoverageMeasure(eventDateToSimDay("1849-12-20"));
    var sep = fillBlockCoverageMeasure(eventDateToSimDay("1849-09-15"));
    var problems = [];
    var gates = [
      { band: "core",       mean: 0.35, tol: FILL_COV_TOL.core,   minN: 3, spreadMin: 0.06 },
      { band: "mid",        mean: 0.12, tol: FILL_COV_TOL.mid,    minN: 3 },
      { band: "fringe",     mean: 0.03, tol: FILL_COV_TOL.fringe, minN: 3 },
      { band: "waterfront", mean: 0.35, tol: FILL_COV_TOL.core,   minN: 1 }
    ];
    gates.forEach(function(g){
      var row = m.perBand[g.band];
      if(!row || row.n < g.minN){
        problems.push(g.band + ": " + (row ? row.n : 0) + " eligible blocks (< " + g.minN + ") — the band is vacuous");
        return;
      }
      if(Math.abs(row.mean - g.mean) > g.tol + 1e-9)
        problems.push(g.band + ": mean coverage " + row.mean + " outside " + g.mean + " ± " + g.tol);
      if(g.spreadMin != null && row.n >= 3 && (row.max - row.min) < g.spreadMin - 1e-9)
        problems.push(g.band + ": per-block spread " + +(row.max - row.min).toFixed(3) + " < " + g.spreadMin +
                      " (blocks must sample the measured in-band range, never one painted mean)");
    });
    var wv = buildFillMaster()._wave || {};
    return { pass: problems.length === 0, law: "blockCoverage (RES-4 measured per-band coverage, ics26)",
             date: "1849-12-20", perBand: m.perBand, sept15Informational: sep.perBand,
             targets: { core: "0.35±" + FILL_COV_TOL.core + " + spread", mid: "0.12±" + FILL_COV_TOL.mid,
                        fringe: "0.03±" + FILL_COV_TOL.fringe, waterfront: "core-grade (the RES-4 M5 wharf-corridor exception)" },
             hotspots: wv.hotspots || [], ledger: wv.coverage || null,
             eligibleBlocks: m.blocks.length, blocks: m.blocks,
             problems: problems.slice(0, 10),
             qaKnobs: { ignoreCoverageTargets: FILL_QA.ignoreCoverageTargets } };
  });
})();
...
function buildTentMaster(){
  ...
  zones.forEach(function(zone){
    ...
    while(list.length < targetMax && tries < maxTries){
      ...
      //     s106e: an EXPANDING front's avoid circle covers its full annex claim
      //     (the expanded depth, centre-shifted into the block) — a tent never
      //     seeds on ground an expansion will lawfully take.
      for(pi = 0; pi < fillM.length; pi++){
        var f = fillM[pi], fdx = x - f.cx, fdz = z - f.cz;
        var fD = f.exp ? f.exp.dExp : f.d;
        var fr = rB + Math.hypot(f.w, fD)/2 + (f.exp ? (fD - f.d)/2 : 0) + 0.5;
...
function tentPartsInto(parts, e){
  /* s106e: a CANVAS-FIRST fill entry (a non-tent catalog code in its canvas
     phase) draws with the TNT-3 framed-cloth-house silhouette — the record's
     own proto-building canvas form (canvas over a frame skeleton, bible §2) —
     never a bespoke new shape. Tent codes keep their own variants. */
  var v = TENT_VARIANTS[e.code] || (e.canvasFirst && TENT_VARIANTS["TNT-3"]) || { wallH: 0.8, ridgeH: 1.2, tone: TENT_TONE_DEFAULT };
  var g = groundStructureAt(e.cx, e.cz, e.w, e.d, e.yaw);
  var y = g.floorY;
  parts.push(groundPlinthGeo(g, new THREE.Color(LM_PLACEHOLDER.pad)));
...
if(typeof window !== "undefined"){
  ...
  window.__P1850_FILL_QA = function(opts){
    ...
    if(opts.uncapCoreCanvas != null)   FILL_QA.uncapCoreCanvas = !!opts.uncapCoreCanvas;   // s106d: drop the core-canvas fraction cap (zoneCoreCanvas fail-before)
    if(opts.ignoreSlotGates != null)   FILL_QA.ignoreSlotGates = !!opts.ignoreSlotGates;   // s106e: rear tents posed street-side gateless + expansions without consumption teardowns (lotSlots fail-before)
    if(opts.ignoreCoverageTargets != null) FILL_QA.ignoreCoverageTargets = !!opts.ignoreCoverageTargets;   // ics26: flat plaza-radial wave, no block budgets (blockCoverage fail-before)
    FILL_QA.version++;
    FILL_MASTER = null; TENT_MASTER = null; _fillLastDay = null; _tentLastDay = null;
    return { ignoreInTownGates: FILL_QA.ignoreInTownGates, uncapCoreCanvas: FILL_QA.uncapCoreCanvas,
             ignoreSlotGates: FILL_QA.ignoreSlotGates, ignoreCoverageTargets: FILL_QA.ignoreCoverageTargets,
             version: FILL_QA.version };
  };
}
setTimeout(function(){
  if(typeof window !== "undefined" && window.__P1850){
...
setTimeout(function(){
  if(typeof window !== "undefined" && window.__P1850){
    ...
    /* s106d — the boom-window core-canvas exception, machine-readable (probes/shots). */
    window.__P1850.coreCanvas = function(){
      var m = buildFillMaster(), wv = m._wave || {};
      var list = [];
      for(var i = (wv.v1 || 0); i < m.length; i++){
        var e = m[i]; if(!e.coreCanvas) continue;
        list.push({ id: e.code + "#" + e.rank, x: +e.cx.toFixed(1), z: +e.cz.toFixed(1),
                    appearDay: +e.appearDay.toFixed(1),
                    events: fillLifecycleOf(e).events.map(function(ev){ return ev.type + "@" + ev.start.toFixed(1); }) });
      }
      return { frac: CAD_CORE_CANVAS_FRAC, windowISO: [CAD_CORE_CANVAS_START_ISO, CAD_CORE_CANVAS_END_ISO],
               coreWave: wv.coreWave, coreCanvas: wv.coreCanvas, cap: wv.coreCanvasCap, entries: list };
    };
    /* s106e — the slots/arcs ledger, machine-readable (probes/shots). */
    window.__P1850.lotSlots = function(){
      var m = buildFillMaster(), wv = m._wave || {};
      var rears = [], cvfs = 0, ths = 0, exps = [];
      for(var i = (wv.v1 || 0); i < m.length; i++){
        var e = m[i];
        if(e.canvasFirst) cvfs++;
        if(e.tentHarden) ths++;
        if(e.rearOf != null) rears.push({ id: e.code + "#" + e.rank, rearOf: e.rearOf, slot: e.slot,
                                          x: +e.cx.toFixed(1), z: +e.cz.toFixed(1),
                                          appearDay: +e.appearDay.toFixed(1),
                                          consumedAt: e.consumedAt != null ? +e.consumedAt.toFixed(1) : null });
        if(e.exp) exps.push({ id: e.code + "#" + e.rank, x: +e.cx.toFixed(1), z: +e.cz.toFixed(1),
                              start: +e.exp.start.toFixed(1), on: +e.exp.on.toFixed(1),
                              dExp: +e.exp.dExp.toFixed(2), consumes: e.exp.consumes });
      }
      return { lots: wv.lots, lotsWithRear: wv.lotsWithRear, rearSlots: wv.rearSlots, rearCap: wv.rearCap,
               canvasFirst: cvfs, tentHarden: ths, expansions: exps.length, rears: rears, exps: exps,
               cvfWindowD: [FILL_RES_CONV_MIN_D, FILL_RES_CONV_MAX_D], cvfFrac: FILL_CVF_FRAC,
               tentHardenFrac: FILL_TENT_HARDEN_FRAC, rearFrac: FILL_REAR_FRAC, expFrac: FILL_EXP_FRAC };
    };
    ...
    /* ics26 — the measured per-block coverage table (probe/screening evidence;
       read-only over the shared measure the blockCoverage audit gates). */
    window.__P1850.blockCoverage = function(iso){
      var day = iso ? eventDateToSimDay(iso) : eventDateToSimDay("1849-12-20");
      var m = fillBlockCoverageMeasure(day);
      var wv = buildFillMaster()._wave || {};
      return { date: iso || "1849-12-20", perBand: m.perBand, blocks: m.blocks,
               hotspots: wv.hotspots || [], ledger: wv.coverage || null,
               fencedLots: wv.fencedLots, fenceSegs: wv.fenceSegs,
               fences: FILL_FENCE_RENDER.map(function(fb){
                 return { x: +fb.segs[0].x0.toFixed(1), z: +fb.segs[0].z0.toFixed(1),
                          segs: fb.segs.length, appearDay: +fb.appearDay.toFixed(1) };
               }) };
    };
  }
}, 0);
