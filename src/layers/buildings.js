...
/* seeded weighted draw of a concrete code within a census category, era-valid at
   ...
   exception admits CANVAS, never the scrap-shanty tier). */
function fillPickCode(cat, day, rng, snapKey, tentOnly){
  var pool = [], tot = 0;
...
var FILL_QA = { ignoreInTownGates: false, uncapCoreCanvas: false, ignoreSlotGates: false, version: 0 };
...
var FILL_REAR_FRAC        = 0.25;         // cap: rear-slot placements <= ceil(frac x shanty-bucket placements) (streaming, like the core-canvas cap)
  ...
  while(waveN < need2){
    ...
    if(cat2 === "shanty" && !coreCanvasDone && !FILL_QA.ignoreInTownGates &&
      ...
      for(; ct && ci < slots2.length; ci++){
        ...
        if(!ccp.ok) continue;
        if(fillQuadCrossesRow(fillPoseQuad(ccx, ccz, cwM, cdM, cs.yaw), ccx, ccz)) continue;
        var cResBad = false;
    ...
    if(cat2 === "shanty" && lots.length &&
      ...
      for(var li = 0; rt && li < lots.length && !placedRear; li++){
        var lot = lots[li];
        ...
        if(!FILL_QA.ignoreSlotGates){
          if(fillCoreBlockAt(rcx, rcz, waveCap)) rBad = true;                                        // rear tents are residential texture
            ...
            if(!rcp.ok) rBad = true;
            else if(fillQuadCrossesRow(fillPoseQuad(rcx, rcz, rw, rd, fe.yaw), rcx, rcz)) rBad = true;
            else {
        ...
        if(fillOverlapsLocal(placed, rcx, rcz, rB)){ lot.slotBad[slotK] = true; continue; }
        counts2.shanty++; waveN++; rearN++;
        placed.push({ x: rcx, z: rcz, r: rB });
...
if(typeof window !== "undefined"){
  ...
  window.__P1850_FILL_QA = function(opts){
    ...
    return { ignoreInTownGates: FILL_QA.ignoreInTownGates, uncapCoreCanvas: FILL_QA.uncapCoreCanvas,
             ignoreSlotGates: FILL_QA.ignoreSlotGates, version: FILL_QA.version };
  };
}
setTimeout(function(){
  if(typeof window !== "undefined" && window.__P1850){
