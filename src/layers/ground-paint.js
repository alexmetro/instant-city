...
/* CARTOGRAPHIC MIN-WIDTH FLOOR (operator 2026-07-17): a built road never PAINTS
   thinner than this, so the 2 m trail class stops reading as a pencil line next
   to the 21 m mains. The SURVEYED width (s.widthM — the ROW that lots, routing,
   and placement read) is untouched TRUTH; this is a render-legibility floor on
   the painted ribbon only, exactly as a real map draws a narrow footpath at a
   visible minimum. Applies to established (cls 2) built roads; the S1 ghost
   survey scratch keeps its own hairline weight. */
var GP_MIN_PAINT_W = 5.5;
function gpPaintWidthM(s){ return (s.cls === "pier") ? s.widthM : Math.max(s.widthM, GP_MIN_PAINT_W); }
...
function renderGroundSplat(){
  ...
  STREETS_RUNTIME.forEach(function(s){
    ...
    function flush(end){
      ...
      var run = wpts.slice(runA, end + 2); run._wM = gpPaintWidthM(s);
      _gpStrokeRun(run, c);
    ...
    if(ext.trimmedAt && cls.length >= 1){
      ...
      if(cls[capSeg] === 2){
        ...
        if(cdl > 0.01){
          cdx /= cdl; cdz /= cdl;
          var capL = Math.min(2.0, cdl), chw = gpPaintWidthM(s) / 2, cnx = -cdz, cnz = cdx;
          _gpFillPoly([
...
registerAudit("ground-paint", "constantWidth", function(){
  ...
  _members.forEach(function(s){
    ...
    var pw = gpPaintWidthM(s); // measure against the PAINTED width (surveyed ROW, floored for legibility)
    var tolAbs = Math.max(2.5, 0.35 * pw);
    if(widths.some(function(w){ return Math.abs(w - pw) > tolAbs; }) || (mx - mn) > Math.max(2.0, 0.30 * pw)){
      out.pass = false; out.violations.push({ id:s.id, widthM:s.widthM, paintedTarget:+pw.toFixed(2), measured:widths });
    }
  });
  return out;
});
