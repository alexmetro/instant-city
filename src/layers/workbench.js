...
(function(){
  ...
  var WB_DOODAD_CELL_BASE = DOODAD_CELL;
  ...
  function applyKnob(key, v){
    ...
    else if(key==="doodadMul"){ DOODAD_CELL = WB_DOODAD_CELL_BASE/Math.sqrt(Math.max(v,0.01)); _doodadBuilt = false; }
    else if(key==="streetAlphaMul"){ [splatWorldMesh, splatTownMesh, splatCloseMesh].forEach(function(m){ m.material.opacity = v; }); }
