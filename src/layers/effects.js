...
function updateFire(dt){
  ...
  if(!windowDays){
    ...
    fireGlow = 0; _fireBurningCount = 0; // s67: zero the QA counter too — __P1850.fire.burning read stale after jumping out of the window
    flameMesh.count = 0; flameMesh.instanceMatrix.needsUpdate = true;
...
/* TWO-MIDDAYS FIX (s67, graphics-campaign.md caveat + D1.1): the entire
   light rig (sun position/color/intensity, sky, fog, nightFactor, window
   glow, water dim, fireGlow, weather/wet tint) was re-derived ONLY inside
   the rAF loop. Every discrete time-set path (scrub release, notch click,
   [ / ], T-skip, __P1850.jump/jumpDay/setHour, URL restore) moved the sim
   clock and then waited for the next animate() frame to relight — so any
   frame rendered before that (audit readbacks via __P1850.render(), CDP
   captures, rAF-throttled/hidden tabs where the loop never runs at all)
   showed the PREVIOUS moment's lighting: a midnight-dark "noon" failing
   the Earth Palette luminance audit at 0.398, or a warm dusk/fire tint
   persisting onto figures after a jump. One sun, one noon: this function
   re-runs the full per-frame light chain synchronously, in animate()'s
   exact order, and core's updateHashDate() — the one choke point every
   discrete jump already commits through — calls it. lightBlend snaps to
   its steady-state target (no cross-fade residue), so ANY path to a given
   sim-hour lands in the identical light state the live loop converges to.
   Audited by effects.lightStateConsistency below. */
function refreshLightState(){
  updateWeather();
  updateWetTint();
  updateFire(0);
  var fireWindow = simDay>=FIRE.day0 && simDay<=FIRE.day1;
  lightBlend = (timelapseActive()||SCRUBBING) && !fireWindow ? 1 : 0; // updateTimeOfDay's own wantRep target, snapped
  updateTimeOfDay(0); // dt=0: chase() is a no-op on the already-snapped blend; timeOfDay derives fresh
  updateDayNight();
}
...
/* ---- effects audit (layers-spec.md rules block; s67 two-middays fix):
   ONE SUN, ONE NOON — any path to a given sim-hour must produce IDENTICAL
   lighting. Reaches the same noon three ways and compares the terrain-
   luminance readback (median over ~28 bare-ground probes):
     1. discrete jump — simDay set + updateHashDate(), the commit choke
        point every explicit jump path funnels through (scrub release,
        notch click, [ / ], T-skip, __P1850.jump/jumpDay/setHour);
     2. live-play arrival — direct clock set + the animate() frame chain
        at a normal frame dt (weather -> wet tint -> fire -> timeOfDay ->
        day/night), exactly what a running loop does;
     3. scrub-drag residue — SCRUBBING through the previous midnight
        (representative-light blend engaged, night phase latched), then
        release-commit to the target noon: dusk/night/rep state must not
        persist through the landing.
   Skips (pass, reason stated) when the frame can't host the probe — same
   conditions as terrain.luminance — or inside the Great Fire burn window,
   where the light state is legitimately dynamic (flame flicker). ---- */
registerAudit("effects", "lightStateConsistency", function(){
  var alt = lastKnownAlt||0;
  if(!(alt>=100 && alt<=1500))
    return { pass:true, skipped:"camera altitude "+Math.round(alt)+"m outside the 100-1500m calibration band" };
  var day0 = simDay, scrub0 = SCRUBBING;
  var target = Math.floor(day0)+0.5; // local noon of the current sim day
  if(target>=FIRE.day0-0.05 && target<=FIRE.day1+1)
    return { pass:true, skipped:"target noon inside the Great Fire window — flame flicker is legitimately dynamic" };
  function lum(){
    var r = __P1850_AUDITS.terrain.luminance();
    return (r && r.median!=null) ? r.median : null;
  }
  function restore(){ simDay = day0; SCRUBBING = scrub0; updateHashDate(); }
  try {
    // path 1 — discrete jump (the updateHashDate -> refreshLightState commit)
    simDay = target; updateHashDate();
    var m1 = lum();
    // path 2 — live-play arrival: the per-frame chain at a normal dt
    simDay = target;
    updateWeather(); updateWetTint(); updateFire(1/60); updateTimeOfDay(1/60); updateDayNight();
    var m2 = lum();
    // path 3 — scrub-drag through the previous midnight, then release-commit
    SCRUBBING = true; simDay = Math.floor(day0);
    updateTimeOfDay(0.05); updateDayNight(); // mid-drag frames: rep blend engaging, night phase latched
    SCRUBBING = false; simDay = target; updateHashDate(); // release commits the jump
    var m3 = lum();
    restore();
    if(m1==null || m2==null || m3==null)
      return { pass:true, skipped:"luminance probe unavailable at target noon (offscreen/too few clear probes)",
               medians:{ jump:m1, live:m2, scrubRelease:m3 } };
    var spread = Math.max(m1,m2,m3) - Math.min(m1,m2,m3);
    return { pass: spread<0.02, spread:+spread.toFixed(4),
             medians:{ jump:+m1.toFixed(3), live:+m2.toFixed(3), scrubRelease:+m3.toFixed(3) } };
  } catch(e){
    restore();
