...
/* ics14c LANDMARK/BUSINESS + LOT INK (operator: parchment-gold business names
   unreadable over sand + grey placeholder roofs). Same polarity family as the
   districts — these voices are hosted on LIGHT surfaces (sand/dirt ground AND
   the placeholder roof greys), so dark ink + light casing — but the ink is
   MEASURED one step darker than the district #453f35: glance voices must
   clear raw ink-vs-host ≥3.0 (audit GLANCE rule) and #453f35 measures 2.69
   on rocky, while WHITE street ink measures 1.83 on the light civic roof
   tint 0xb7c1c7 — neither existing pairing serves this host set. #332e26
   warm sepia-black clears ≥3.48 on every land AND roof background
   (0xb7c1c7 / 0x9ea19c / 0x868984). Ink + casing only (ics14b no-glow law).
   CASING WIDTH (screened, first ics14c pass): at glance sizes a halo:5 light
   casing is as wide as the Georgia stems — the label rasters as a light-
   rimmed emboss and the dark ink loses the mass. The name voices carry a
   THIN casing (halo 3; lot numerals keep 4 — chunky bold digits screened
   crisp) so the measured ink dominates the glyph; the casing's job here is
   only to separate the dark ink on the rare dark-ground overhang. */
var LBL_LANDMARK_INK  = "#332e26";                 // measured glance ink — civic/landmark names + both lot voices
...
var LBL_STYLE_SETS = {
  a: {
    ...
    civic:   { key:"a.civic",   family:"Georgia, 'Times New Roman', serif", weight:"700", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:3, letterSpacing:0.24, smallCaps:true,  italic:false, wrap:8 },
    street:  _lblStreetStyle("a"),
    lotNum:  { key:"a.lotNum",  family:"Georgia, 'Times New Roman', serif", weight:"700", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:4, letterSpacing:0.02, smallCaps:false, italic:false },
    lotOwn:  { key:"a.lotOwn",  family:"Georgia, 'Times New Roman', serif", weight:"500", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:3, letterSpacing:0.04, smallCaps:false, italic:true, wrap:8  }
  },
  b: {
    ...
    civic:   { key:"b.civic",   family:"'Palatino Linotype','Book Antiqua',Palatino,'Times New Roman',serif", weight:"700", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:3, letterSpacing:0.30, smallCaps:true,  italic:false, wrap:8 },
    street:  _lblStreetStyle("b"),   // ics14: the operator's Maps street voice overrides the era voice in every set
    lotNum:  { key:"b.lotNum",  family:"'Palatino Linotype','Book Antiqua',Palatino,'Times New Roman',serif", weight:"700", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:4, letterSpacing:0.02, smallCaps:false, italic:false },
    lotOwn:  { key:"b.lotOwn",  family:"'Palatino Linotype','Book Antiqua',Palatino,'Times New Roman',serif", weight:"500", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:3, letterSpacing:0.04, smallCaps:false, italic:true, wrap:8  }
  },
  c: {
    ...
    civic:   { key:"c.civic",   family:"Didot,'Bodoni MT','Times New Roman',serif", weight:"600", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:3, letterSpacing:0.14, smallCaps:true,  italic:false, wrap:8 },
    street:  _lblStreetStyle("c"),   // ics14: the operator's Maps street voice overrides the era voice in every set
    lotNum:  { key:"c.lotNum",  family:"Didot,'Bodoni MT','Times New Roman',serif", weight:"600", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:4, letterSpacing:0.02, smallCaps:false, italic:false },
    lotOwn:  { key:"c.lotOwn",  family:"Didot,'Bodoni MT','Times New Roman',serif", weight:"400", color:LBL_LANDMARK_INK, haloColor:LBL_DISTRICT_CASE, glow:false, halo:3, letterSpacing:0.03, smallCaps:false, italic:true, wrap:8  }
  }
};
...
registerAudit("labels", "contrast", function(){
  ...
  // ics14 — the surface families each voice is HOSTED on (operator's polarity
  // rule: ink is chosen per host surface; these are our known constants).
  // ics14c: values are SETS — civic/landmark + lot text sits over sand ground
  // AND the grey placeholder roofs (buildings occupy the named lots).
  var HOST_FAM = { street:["road"], region:["land"], hill:["land"], place:["land"],
                   civic:["land","roof"], lotNum:["land","roof"], lotOwn:["land","roof"], water:["water"] };
  // ics14c GLANCE voices: small read-at-a-glance names whose INK BODY must
  // itself clear the host surface (min component, not max — see doc above).
  var GLANCE_INK = { street:1, civic:1, lotNum:1, lotOwn:1 };
  var DETAIL_TABLE = { street:1, place:1, civic:1 };   // the operator-decided voices report full per-bg tables
  ...
  Object.keys(LBL_STYLE).forEach(function(key){
    ...
    // ics14: measure the style's OWN rings (street/districts/civic/lots = ink
    // + casing ONLY, no glow — ics14b/c; water = the classic dark ring +
    ...
    BG.forEach(function(b){
      ...
      var inkVsBg = _lblContrast(glyph, b.rgb);
      ...
      // ics14c GLANCE-INK GATE: on its HOST surfaces a glance voice's ink body
      // must directly clear the ground (a casing cannot carry an invisible ink).
      if(isHost && GLANCE_INK[key]) here = Math.min(here, inkVsBg);
      if(DETAIL_TABLE[key]) perBg.push({ bg:b.name, host:isHost, ringSep:+ringSep.toFixed(2), inkVsCasing:+glyphVsInner.toFixed(2), inkVsBg:+inkVsBg.toFixed(2), score:+here.toFixed(2) });
      if(here < sMin){ sMin = here; sBg = b.name; }
      if(isHost && here < hMin){ hMin = here; hBg = b.name; }
    });
    var row = { style:key, minRatio:+sMin.toFixed(2), worstBg:sBg, hostFam:hostFams,
  ...
  return { pass: bad.length===0, gate:GATE,
           method:"per-style WCAG min over earth palette (ics14c: + roof greys) + per-host-family gate; glow:false voices = ink+casing only, sep=max(casingVsBg, inkVsBg); GLANCE voices (street/civic/lotNum/lotOwn) additionally take inkVsHost as a MIN on host backgrounds; water keeps the dual halo; street Bold shares Medium colors (one measurement, both weights)",
